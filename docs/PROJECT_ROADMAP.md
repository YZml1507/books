...
> **成文于 R1xx 时代（2026-08-15），仅供历史参考**——文中 `llm_reader`、
> `history.db` 写路径等已退役；现状以 `TASK_LEDGER.md` 末轮与 `PHASE.md` 为准。
...
### P3 新功能：六爻占卜 + 黄历择日（本地计算，参照 YiSphere 架构）✅ 已完成（liuyao/huangli tab 接线，R53b 端点 standing 自测覆盖；**R111b 起扩展：桃花运 tab（taohua.py，R111b）+ 塔罗占卜 tab（tarot.py，R112b）+ 大运桃花应期（R113b）+ 塔罗牌阵位置（R114b），R116b 标注；八字合婚 tab（hehun.py，R121b），R123b 标注**）
