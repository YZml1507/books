...
## D-198b R152b 优化轨：web standing 自测缺口——bazi range 倒序/超31天 + bookstudy chapter scheme 空 + qiming 计算失败四条 400 校验分支零断言（能力层验证，与 R139b-R151b 同族——同端点不同校验维度）

**背景（亲自核实）**：R139b-R151b 已按端点逐个补 err.* 400 断言，但
**同端点剩余校验维度**仍零断言：
- bazi range 路径的**运行时校验**（calc_range，web/app.py:217 捕获
  ValueError → 400）：倒序（end 早于 start）、超 31 天——R151b 已补
  range_missing/range_format（输入形状校验），但这两条**值域校验**零
  断言（missing/format 断言不触发 calc_range 内部 ValueError）。
- bookstudy chapter 的 scheme 空校验（web/app.py:730 "scheme 不能为
  空"）零断言——err.bookstudy.chapter.empty（R148b）只测 work_id 空，
  scheme 空分支零断言。
- qiming 的**计算失败**分支（web/app.py:914 "起名计算失败：{exc}"，
  name_candidates 抛异常→400）零断言——err.qiming.*（R140b/R149b）
  只测参数校验，计算失败路径零断言。
- 四条分支均零 standing 断言（L-22/L-23 同族；与 R139b-R151b 同族
  ——同端点不同校验维度）。

**实测数据（命令实跑，web TestClient）**：
- `POST /api/bazi {scope:"range", range_start:"2026-02-01",
  range_end:"2026-01-01"}` → 400，detail "end_date 不能早于 start_date"
- `POST /api/bazi {scope:"range", range_start:"2026-01-01",
  range_end:"2026-03-15"}` → 400，detail "日期范围最长 31 天（一次查询
  防爆），请分段查询"
- `GET /api/bookstudy/chapter?work_id=KR1a0001&scheme=` → 400，detail
  "scheme 不能为空"
- `POST /api/qiming {month:2, day:30, ...}` → 400，detail "起名计算
...
| 方案 | 内容 | 实测/风险 |
|---|---|---|
| **A（选定）** | web/app.py --selftest 的 err.* 区块补四条断言：err.bazi.range_order（倒序→400）、err.bazi.range_span（超31天→400）、err.bookstudy.chapter.scheme（scheme 空→400）、err.qiming.calc_fail（month=2/day=30→400）（95→99 checks） | 纯加自测断言、零功能改动/零数据风险；补上 bazi range 值域校验、bookstudy scheme 空、qiming 计算失败四个未覆盖的 400 分支，抓校验/计算静默失效；与 R139b-R151b 同族（同端点不同校验维度），确定性可复验 |
| B | 补 MCP research_tool 深度验证 | MCP selftest 需协议级 subprocess，工作量大；且 research_tool 与 web /api/research 同源、web 侧已有 research.allow_damaged 断言 |
| C | 补 tarot 端点校验断言 | tarot 为概率性端点（seed 驱动），参数校验维度少，确定性弱于 A 的参数校验 |

选 A（补 err.bazi.range_order/range_span + err.bookstudy.chapter.scheme
+ err.qiming.calc_fail 四条 400 断言，照 R139b-R151b 先例：能力路径
...
## D-211b R165b 优化轨：MCP standing 自测缺口——research_tool 协议级断言零覆盖（能力层验证，与 R144b/R146b web 侧 research 断言同源——web 已覆盖 q 空/过长/max_addresses，MCP 侧 research_tool 同内核但协议级零断言）

**背景（亲自核实）**：R139b-R164b 已把 web 层 standing 断言补到 117
checks（66 条 err.* 覆盖 59 条 400 + 3 条 422 分支，能力层 web 封顶）。
但 **MCP 侧 research_tool 协议级断言零覆盖**：mcp_server.py --selftest
的 calls 列表只覆盖 bookstudy_structure/bookstudy_chapter/
compare_works_tool/book_summary_tool/add_local_work_tool/record_claim_tool
六个工具——research_tool（与 web /api/research 同内核，web 侧已有
research.allow_damaged/empty/too_long 断言）**从未被协议级调用断言**。
若 MCP research_tool 的 G7 拒绝路径、max_addresses 钳制、正常检索
回归（500/行为改变），五层自测中 mcp 层看不见（L-22/L-23 同族；
与 R144b err.research.max_addresses / R146b err.research.too_long
同源——同一内核不同发布面）。

**实测数据（命令实跑，直接调 research_tool）**：
- `research_tool(q=" ")` → 返回 REFUSED（G7 拒绝，steps 0/0，无 evidence）
- `research_tool(q="甲"*201)` → 正常返回（MCP 侧无 200 字符上限校验，
  直接进检索——与 web 侧 q 过长→400 不同，是 MCP 设计行为非缺陷）
- `research_tool(q="潛龍勿用", max_addresses=0)` → 钳制为 1，正常返回
- `research_tool(q="潛龍勿用", max_addresses=2)` → 正常 evidence
- 四个行为均可确定性断言——补协议级断言零风险（mcp selftest 已有
  subprocess 协议框架，加 tools/call 条目即可）。

**候选方案**：

| 方案 | 内容 | 实测/风险 |
|---|---|---|
| **A（选定）** | mcp_server.py --selftest 的 calls 列表补 research_tool 协议级断言：q=" "→REFUSED（G7 拒绝路径）、q=潛龍勿用→evidence 正常路径（同内核 web 断言已在，协议面补覆盖） | 纯加协议级自测断言、零功能改动/零数据风险；补上 MCP research_tool 的 G7 拒绝/正常检索协议级覆盖（web 侧断言不覆盖 MCP 发布面），抓 MCP 行为静默回归；与 R144b/R146b 同源（同一内核不同发布面），确定性可复验 |
| B | 补 MCP research_tool max_addresses 钳制断言 | 钳制是 min/max 表达式行为（max_addresses=0→1），与 G7 拒绝/正常路径相比确定性等价，可并入 A 一并断言 |
| C | 文档滞后扫描继续 | R161b 已修 tab 表述、checks 数逐轮同步，无新滞后点 |

选 A（补 MCP research_tool 协议级断言：G7 拒绝 + 正常检索两条，照
