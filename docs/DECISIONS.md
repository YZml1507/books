...
## D-096a R31a 审查轨：R30a pending 清理 + rebase 纳入 R59b-R60b（2026-08-17）

- **接续 R30a**：fetch 发现 HEAD..origin/main 仍显示 R59b/R60b pending。
  根因：R30a 处理纯文档轮时选择"不 rebase"，导致 R59b/R60b 虽已台账记录
  （§76）但未纳入 audit 分支 history。
- **修正 rebase**：本轮执行 `git rebase origin/main`，在历史 commit
  e189795（R22a rebase merge）处 append-only docs/ 冲突。按用户指令
...
## D-107a R97a 审查轨：纯文档轮 rebase 纳入 R94b，清空 pending（2026-08-17）

- **接续 R96a**：fetch origin 成功（网络已恢复），HEAD..origin/main 显示
  优化轨推进 1 提交（2da7c2d R94b）。`git show --name-only` 确认纯
  docs/*.md only（DECISIONS + TASK_LEDGER）。按协议第 3 步走纯文档轮，
  不启动审查循环。
...
- **rebase**：`git rebase origin/main` 在历史 a70bd0e（R22a merge）处
  append-only docs/ 冲突（DECISIONS + TASK_LEDGER）。按既定协议"冲突取
  --theirs"：`git checkout --theirs docs/*.md` → `git add` →
  `GIT_EDITOR=true git rebase --continue`。rebase 成功，R94b 纳入
  audit 分支 history，HEAD..origin/main 清空。
- **领土零越界**：rebase 后 `git diff origin/main..HEAD`：审查轨领土
  `scripts/assess_goals.py` 有改动（R21a 委托修复 8c1242c，历史遗留合法）；
  优化轨领土 `src/guji/**` `web/**` 审查轨 diff 为空（0 字节）→ 领土零越界确认。
...
## D-108a R98a 审查轨：纯文档轮 rebase 纳入 R95b-R96b，清空 pending（2026-08-17）

- **接续 R97a**：fetch origin 成功，HEAD..origin/main 显示优化轨推进 2 提交
  （3018e07 R95b、04823bb R96b）。`git show --name-only` 确认全 docs/*.md only
  （PROJECT_STATUS/GOAL_NEXT_SESSION/DECISIONS/TASK_LEDGER）。按协议第 3 步
  走纯文档轮，不启动审查循环。
- **逐行复审 2 提交 diff**（亲眼过）：R95b sync PROJECT_STATUS 快照块日期标签
  2026-08-16→2026-08-17（R78b 改行 19 漏改块标签，`git log -L 19,19` 实测，
  D-130b 先例）；R96b note GOAL_NEXT_SESSION §0a 本窗口会话 jsonl 路径
  `1ae2121e85ce8e84/`（含 71672968…jsonl 与交接话一致，旧窗口保留回溯，
  `ls .atomcode/sessions/` 实测两目录都存在）。均归因诚实，无越界。纪律良好。
- **rebase**：`git rebase origin/main` 在历史 bcc6cc6（R22a merge）处
  append-only docs/ 冲突（DECISIONS + TASK_LEDGER）。按既定协议"冲突取
  --theirs"：`git checkout --theirs docs/*.md` → `git add` →
  `GIT_EDITOR=true git rebase --continue`。rebase 成功，R95b/R96b 纲入
  audit 分支 history，HEAD..origin/main 清空。
- **领土零越界**：rebase 后 `git diff origin/main..HEAD`：审查轨领土
  `scripts/assess_goals.py` 有改动（R21a 委托修复 8c1242c，历史遗留合法）；
  优化轨领土 `src/guji/**` `web/**` 审查轨 diff 为空（0 字节）→ 领土零越界确认。
...
## D-109a R99a 审查轨：纯文档轮 rebase 纳入 R97b，清空 pending（2026-08-17）

- **接续 R98a**：fetch origin 成功，HEAD..origin/main 显示优化轨推进 1 提交
  （4ccc129 R97b）。`git show --name-only` 确认纯 docs/*.md only
  （GOAL_NEXT_SESSION/DECISIONS/TASK_LEDGER）。按协议第 3 步走纯文档轮，
  不启动审查循环。
- **逐行复审 R97b diff**（亲眼过）：GOAL_NEXT_SESSION §0a sessionID
  ...
  路径注释没补表行）。归因诚实，无越界。纪律良好。
- **rebase**：`git rebase origin/main` 在历史 31c1b7b（R22a merge）处
  append-only docs/ 冲突（DECISIONS + TASK_LEDGER）。按既定协议"冲突取
  --theirs"：`git checkout --theirs docs/*.md` → `git add` →
  `GIT_EDITOR=true git rebase --continue`。rebase 成功，R97b 纳入
  audit 分支 history，HEAD..origin/main 清空。
- **领土零越界**：rebase 后 `git diff origin/main..HEAD`：审查轨领土
  `scripts/assess_goals.py` 有改动（R21a 委托修复 8c1242c，历史遗留合法）；
  优化轨领土 `src/guji/**` `web/**` 审查轨 diff 为空（0 字节）→ 领土零越界确认。
...
## D-110a R100a 审查轨：纯文档轮 rebase 纳入 R98b-R99b，清空 pending（2026-08-17）

- **接续 R99a**：fetch origin 成功，HEAD..origin/main 显示优化轨推进 2 提交
  （a416b18 R98b、b0c2928 R99b）。`git show --name-only` 确认全 docs/*.md only
  （GOAL_NEXT_SESSION/GOAL/DECISIONS/TASK_LEDGER）。按协议第 3 步走纯文档轮，
  不启动审查循环。
- **逐行复审 2 提交 diff**（亲眼过）：R98b sync GOAL_NEXT_SESSION §2a R21a
  委托 commit 引用 23d0f94→337aadc（`git merge-base --is-ancestor 23d0f94
  origin/audit/R18` 实测失败，审查轨历轮 rebase 重写哈希，strike 失效引用照
  D-008；与台账/DECISIONS R21a 委托修复 8c1242c/337aadc 一致）；R99b annotate
  GOAL §1 git push 授权例外（历次窗口用户已授权 push 到 main 见 GOAL_NEXT_SESSION
  §4，重写历史仍红线，补文档内部矛盾）。均归因诚实，无越界。纪律良好。
...
## D-111a R101a 审查轨：纯文档轮 rebase 纳入 R100b，清空 pending（2026-08-17）

- **接续 R100a**：fetch origin 成功，HEAD..origin/main 显示优化轨推进 1 提交
  （8ada404 R100b）。`git show --name-only` 确认纯 docs/*.md only
  （GOAL_NEXT_SESSION/DECISIONS/TASK_LEDGER）。按协议第 3 步走纯文档轮，
  不启动审查循环。
- **逐行复审 R100b diff**（亲眼过）：GOAL_NEXT_SESSION §0a session 表本窗口
  ...
  先例，与 R92b/R86b 同族）。归因诚实，无越界。纪律良好。
- **rebase**：`git rebase origin/main` 在历史 e293d4b（R22a merge）处
  append-only docs/ 冲突（DECISIONS + TASK_LEDGER）。按既定协议"冲突取
  --theirs"：`git checkout --theirs docs/*.md` → `git add` →
  `GIT_EDITOR=true git rebase --continue`。rebase 成功，R100b 纳入
  audit 分支 history，HEAD..origin/main 清空。
- **领土零越界**：rebase 后 `git diff origin/main..HEAD`：审查轨领土
  `scripts/assess_goals.py` 有改动（R21a 委托修复 8c1242c/337aadc，历史遗留
  合法）；优化轨领土 `src/guji/**` `web/**` 审查轨 diff 为空（0 字节）→
...
## D-112a R102a 审查轨：纯文档轮 rebase 纳入 R102b-R104b，清空 pending（2026-08-17）

- **接续 R101a**：fetch origin 成功，HEAD..origin/main 显示优化轨推进 2 提交
  （55d76ed R102b、8db1d8e R103b）。逐文件核实全 docs/*.md only（GOAL/DECISIONS/
  TASK_LEDGER）。逐行复审后走 rebase，rebase 后 HEAD..origin/main 又现新提交
  R104b——优化轨高频并行，一并吸收本轮闭环。按协议第 3 步走纯文档轮，
  不启动审查循环。
- **逐行复审 3 提交 diff**（亲眼过）：R102b annotate GOAL §4 T2-T6 stale 旧结论
  逐条 strike 标注实测处置源（T2 G5 PASS summarise_diff exit 0、T3 contiguous/
  suspect 列已落地 unit 表实测 verify_index T10 断言、T4 焦氏易林已入索引 yilin
  5032 单元 §1278、T5 A-12 DONE D-029、T6 G8 PASS knowledge.db 三类隔离），照
  D-008 保留原文，归因诚实；R103b sync GOAL §0 session jsonl 路径旧 .claude\projects\
  →现行 ~\.atomcode\sessions\（本窗口 1ae2121e85ce8e84 旧窗口 025973b91a55cfb5
  回溯），补 R96b/R97b 同族漏同步；R104b de-pin GOAL_NEXT_SESSION §0a session
  表本窗口行轮数"25 �轮"→"以 git log 2946a8a..HEAD | wc -l 实测为准"（实测现
  29 �轮），根治轮数钉死复发（D-146b/D-132b 先例，与 R100b/R92b/R86b 同族）。
  均归因诚实，无越界，无代码逻辑。纪律良好。
- **rebase**：`git rebase origin/main` 历史必在 R22a merge commit 处遇 append-only
  docs/ 冲突。本轮因优化轨高频并行提交，rebase 共执行 2 次：第 1 次 d84a4f7 冲突、
  第 2 次 9a4e1a4 冲突，每次按既定协议"冲突取 --theirs"（`git checkout --theirs
  docs/DECISIONS.md docs/TASK_LEDGER.md` → `git add` →
  `GIT_EDITOR=true git rebase --continue`）。2 次 rebase 成功，R102b-R104b 3 提交
  纳入 audit 分支 history，HEAD..origin/main 清空。
- **领土零越界**：rebase 后 `git diff origin/main..HEAD`：审查轨领土
  `scripts/assess_goals.py` 有改动（R21a 委托修复 8c1242c/337aadc，历史遗留
  合法）；优化轨领土 `src/guji/**` `web/**` 审查轨 diff 为空（0 字节）→
...
## D-114a R104a 审查轨：纯文档轮 rebase 纳入 R106b，清空 pending（2026-08-17）

- **接续 R103a**：fetch origin 成功，HEAD..origin/main 显示优化轨推进 1 提交
  （6eb7d78 R106b）。`git show --name-only` 确认纯 docs/*.md only
  （GOAL/DECISIONS/TASK_LEDGER）。按协议第 3 步走纯文档轮，不启动审查循环。
...
- **rebase**：`git rebase origin/main` 在历史 bb3fbc9（R22a merge）处
  append-only docs/ 冲突（DECISIONS + TASK_LEDGER）。按既定协议"冲突取
  --theirs"：`git checkout --theirs docs/*.md` → `git add` →
  `GIT_EDITOR=true git rebase --continue`。rebase 成功，R106b 纳入
  audit 分支 history，HEAD..origin/main 清空。
- **领土零越界**：rebase 后 `git diff origin/main..HEAD`：审查轨领土
  `scripts/assess_goals.py` 有改动（R21a 委托修复 8c1242c/337aadc，历史遗留
  合法）；优化轨领土 `src/guji/**` `web/**` 审查轨 diff 为空（0 字节）→
...
## D-115a R105a 审查轨：纯文档轮 rebase 纳入 R107b，清空 pending（2026-08-17）

- **接续 R104a**：fetch origin 成功，HEAD..origin/main 显示优化轨推进 1 提交
  （72be5db R107b）。`git show --name-only` 确认纯 docs/*.md only
  （GOAL/DECISIONS/TASK_LEDGER）。按协议第 3 步走纯文档轮，不启动审查循环。
...
- **rebase**：`git rebase origin/main` 在历史 e1387cc（R22a merge）处
  append-only docs/ 冲突（DECISIONS + TASK_LEDGER）。按既定协议"冲突取
  --theirs"：`git checkout --theirs docs/*.md` → `git add` →
  `GIT_EDITOR=true git rebase --continue`。rebase 成功，R107b 纳入
  audit 分支 history，HEAD..origin/main 清空。
- **领土零越界**：rebase 后 `git diff origin/main..HEAD`：审查轨领土
  `scripts/assess_goals.py` 有改动（R21a 委托修复 8c1242c/337aadc，历史遗留
  合法）；优化轨领土 `src/guji/**` `web/**` 审查轨 diff 为空（0 字节）→
...
## D-118a R108a 审查循环：rebase 纳入 R111b/R112b/R113b（taohua 桃花运 + tarot 塔罗 + dayun 应期），逐行复审 + 13 闸门 + web --selftest 32 checks 全绿（2026-08-17）

- **接续 R107a**（323def0，上轮已 push 闭环）。fetch origin 后 ls-remote
  监控发现优化轨推进 main：origin/main HEAD 从 4be962f 变为 c0a3dde。
  HEAD..origin/main 显示优化轨推进 3 提交（b3510f2 R111b、af5fbb3
...
## D-120a R110a 审查循环：rebase 纳入 R115b（ask 模型标注 llm dict）+ R116b（docs sync 24→35），逐行复审 + 13 闸门 + web --selftest 35 checks 全绿（2026-08-17）

## D-121a R111a 审查循环：rebase 纳入 R117b（docs roadmap）+ R118b（修复 huangli affair 两个真实 bug），逐行复审 + 13 闸门 + web --selftest 37 checks 全绿（2026-08-17）
...
## D-126a R116a 审查循环：rebase 纳入 R126b（bazi range/life scopes standing + 修复 history.detail id-source bug）+ R127b（docs sync 45→47），逐行复审 + 13 闸门 + web --selftest 47 checks 全绿（2026-08-17）

- **接续 R115a**（fa999e0，上轮已 push 闭环）。fetch origin 后 ls-remote
  监控发现优化轨推进 main：origin/main HEAD 从 5425550 变为 1a395eb。
  HEAD..origin/main 显示优化轨推进 2 提交（5e05b14 R126b、
  1a395eb R127b）。
- **R126b 改动文件**：docs/{DECISIONS,TASK_LEDGER}.md、web/app.py。
  **含代码逻辑且修复一个真实 bug → 按 §0.3 协议第 5 步立即启动审查
  轨循环，重点逐行复审 bug 修复。**
...
- 13 闸门亲跑全绿（rebase 后 confirm 无回归）：check_quality PASS、
  ...
  (47 checks)**：含 R126b bazi.range + bazi.life 两条新断言（且
  history.detail id-source bug 已修复）。
...
- **接续 R111a**（09c19ce，上轮已 push 闭环）。fetch origin 后 ls-remote
  ...
  ed687a8 R120b）。
- **R119b 改动文件**：docs/{DECISIONS,TASK_LEDGER}.md、web/app.py。
  **含代码逻辑（优化轨领土 web/app.py self-test standing 断言加强）
  → 按 §0.3 协议第 5 步立即启动审查轨循环。**
...
- 13 闸门亲跑全绿（rebase 后 confirm 无回归）：check_quality PASS、
  ...
  liuyao_najia）全 PASS、eval_g1 PASS（246/248）、eval_g4 PASS（yilin
  520/490）、eval_g7 PASS（FABRICATIONS 0）。**web --selftest PASS
  (39 checks)**：含 R119b bazi.lunar + bazi.lunar_leap 两条新断言。

- **接续 R110a**（31809c5，上轮已 push 闭环）。fetch origin 后 ls-remote
  监控发现优化轨推进 main：origin/main HEAD 从 2b021af 变为 aef93a7。
  HEAD..origin/main 显示优化轨推进 2 提交（41b99b5 R117b、
  aef93a7 R118b）。
- **R117b 纯文档轮**：docs/{DECISIONS,PROJECT_ROADMAP,TASK_LEDGER}.md。
  PROJECT_ROADMAP P3 history-extension subitem 标注 not implemented。
  归因诚实，无越界。无红线。
- **R118b 含代码逻辑且修复两个真实 bug**：docs/{DECISIONS,TASK_LEDGER}.md、
  src/guji/huangli.py、web/app.py。**按 §0.3 协议第 5 步立即启动审查
  轨循环，重点逐行复审 bug 修复。**
  - **Bug 1：`/api/huangli?affair=...` 抛 NameError（timedelta 未导入，
    app.py:30）**。修复：`web/app.py` 第 30 行 `from datetime import
    date, datetime` → `from datetime import date, datetime, timedelta`。
  ...
  - **Bug 2："婚嫁" 永远匹配 0 天（前端选项词"婚嫁" vs 宜列表词"嫁娶"）**。
    ...
    {"婚嫁": "嫁娶", "开市": "开市"}`；`find_good_days` 用
    `key = AFFAIR_ALIASES.get(affair, affair)` 归一后再匹配
    `if key in q["yi"]`。验证：`/api/huangli?affair=婚嫁&date=2026-08-17
    &days=30` → good_days 非空。
  - **self-test 加强**（35→37 checks）：`check("liuyao.time", ...)`
    time 起卦 2026-08-16 10:00 → 萃45（gua_number=45）；
    `check("huangli.affair", ...)` affair=婚嫁 2026-08-17 起 30 天 →
    good_days 非空。
  - **复审结论**：Bug 1 修复正确（import 补全），无副作用。Bug 2 修复
    正确（别名归一映射），`AFFAIR_ALIASES` 写死静态，无红线。self-test
    加两条 standing 断言（liuyao.time, huangli.affair），抓端点静默失效。
    无 SQL 注入、无 subprocess、无 eval、无路径拼接风险。**这两个 bug
    此前 13 闸门与五层自测都看不见——R118b 用 standing 断言抓出来修复，
    是优化轨领土的正确修复。**
- **rebase**：stash 数据库产物 → `git rebase origin/main` 在历史
...
- 13 闸门亲跑全绿（rebase 后 confirm 无回归）：check_quality PASS、
  ...
  liuyao_najia）全 PASS、eval_g1 PASS（246/248）、eval_g4 PASS（yilin
  520/490）、eval_g7 PASS（FABRICATIONS 0）。**web --selftest PASS
  (37 checks)**：含 R118b liuyao.time + huangli.affair 两条新断言。

- **接续 R109a**（131437e，上轮已 push 闭环）。fetch origin 后 ls-remote
  监控发现优化轨推进 main：origin/main HEAD 从 fd98167 变为 2b021af。
  HEAD..origin/main 显示优化轨推进 2 提交（ecd2dfa R115b、
  2b021af R116b）。
- **R115b 改动文件**：docs/DECISIONS.md、docs/TASK_LEDGER.md、
  src/guji/llm_reader.py、web/app.py、web/static/index.html。
  **含代码逻辑（优化轨领土 src/guji/llm_reader.py + web 前端）
  → 按 §0.3 协议第 5 步立即启动审查轨循环。**
- **逐行复审 R115b 优化轨领土文件**（亲眼过，优化轨领土 src/guji/web
  ...
  - **web/static/index.html**：ask tab 渲染 LLM 解读时，从
    ...
    正确。无红线。
- **R116b 纯文档轮**：docs/{DECISIONS,GOAL_NEXT_SESSION,PROJECT_ROADMAP,
  PROJECT_STATUS,TASK_LEDGER}.md。同步 web checks 24→35、术数功能列表、
  ROADMAP P3、PROJECT_STATUS。归因诚实，无越界。无红线。
- **rebase**：stash 数据库产物 → `git rebase origin/main` 在历史
...
- 13 闸门亲跑全绿（rebase 后 confirm 无回归）：check_quality PASS、
  ...
  liuyao_najia）全 PASS、eval_g1 PASS（246/248）、eval_g4 PASS（yilin
  520/490）、eval_g7 PASS（FABRICATIONS 0）。**web --selftest PASS
  (35 checks)**：含 R115b ask.llm.shape 一项新断言。
...
- **逐行复审 R111b/R112b/R113b 优化轨领土文件**（亲眼过，优化轨领土
  ...
  - **R111b src/guji/taohua.py**（新 113 行）：纯坐标计算——咸池（桃花）
    年支查三合局；红鸾 `(3 - 年支idx) mod 12`；天喜红鸾对冲 `+6 mod 12`。
    写死说明文字（照 huangli YIJI 静态表先例），无 LLM 生成、无吉凶断言。
    固定八字 → 固定输出，可命令复验。无红线。
  - **R112b src/guji/tarot.py**（新 138 行）：78 张静态牌表（22 大阿卡纳
    + 56 小阿卡纳）。`draw(seed, n)` 用 `random.Random(seed)` 确定性抽牌
    （照 liuyao `cast_coins(rng)` 先例），固定 seed → 固定牌面，可命令
    复验（seed=42 → 节制/皇后/权杖国王）。牌意只给传统公版象征关键词，
    不生成 LLM 解读、不作吉凶断言。无红线。
  - **R113b src/guji/taohua.py dayun_hits**（+28 行）：复用
    `bazi_calc.calc_life` 大运表，查大运地支 == 桃花支（咸池）的应期，
    纯坐标计算，固定八字 → 固定应期。无红线。
  - **web/app.py**：R111b 加 `POST /api/taohua`；R112b 加 `POST /api/tarot`；
    R113b `/api/taohua` 加 dayun_hits 字段。异常处理用
    `HTTPException(422, ...)`。self-test 加 taohua/taohua.dayun/tarot
    standing 断言（29→32 checks）。无红线。
  - **web/static/index.html**：R111b 加 `view-taohua` tab；R112b 加
    `view-tarot` tab；R113b 加"大运桃花应期"表格。前端 fetch 提交表单，
    结果用 esc 转义渲染。无红线。
...
- 13 闸门亲跑全绿（rebase 后 confirm 无回归）：check_quality PASS、
  ...
  liuyao_najia）全 PASS、eval_g1 PASS（246/248）、eval_g4 PASS（yilin
  520/490）、eval_g7 PASS（FABRICATIONS 0）。**web --selftest PASS
  (32 checks)**：含 R110b addr.bcv/addr.yilin/addr.booksec/addr.play/
  addr.euclid 五项 + R111b taohua + R112b tarot + R113b taohua.dayun
  八项新断言。
...
## D-128a 闸门 1「人工点数」落成脚本 `scripts/count_open_findings.py`（R118a）

**问题**：`docs/PHASE.md` 闸门 1 的复验命令那格原本写「人工点数 + probe 引用」。
人工点数不是可复现命令，直接违反宪法第一条「任何状态断言必须附一条能复现它
的命令」。而且**实测会数错**：裸
`Select-String -Pattern '^- 状态：OPEN' | Measure-Object -Line` 得 **10**，
真实 OPEN 条目是 **9** —— 多命中的是 AUDIT_FINDINGS.md 自己的「条目格式」
示范块。翻阶段最关键的那个数字，用最直观的命令去数就是错的。

**候选方案**：

- **A（选定）** 写 `scripts/count_open_findings.py`：只识别 `### R<n>a-<seq>`
  开头的真实条目，解析每条的「状态」与「严重级」，OPEN∩{BLOCKER,MAJOR}
  非空即退出码 1。
- B 把示范块挪进注释/独立文件，让裸 grep 变准。
- C 保持人工点数，靠纪律不数错。

**选定 A 的理由**：闸门必须有非零退出码（宪法第四条 U-08：`probe_bcv.py` 曾
永远返回 0，包括它打印「56/66 卷」那次）。A 提供退出码，B 只提供一个更容易
再次跑偏的正则——文档格式一变（比如以后加个 MINOR 示范）就又错了。A 还顺带
打印逐条清单，翻阶段时可直接贴进 PHASE.md 登记。

**否决 B 的原因**：治标。裸 grep 的脆弱性来自「用文本模式近似结构」，改文档
只是把这次的误命中挪走，下次换个写法照样中招。
**否决 C 的原因**：这正是宪法第一条列为严重违规的「凭印象写结论」。本项目
9 条先例全是口头结论被实测推翻。翻阶段是不可逆动作，不能建在人工点数上。

**实测**：`<py> scripts\count_open_findings.py` → 退出码 1，
末行 `OPEN BLOCKER 4 / OPEN MAJOR 5 / 合计 9`，逐条清单 9 行。

## D-129a 订正 PHASE.md 闸门 2 指向的不存在文件（R118a）

**问题**：闸门 2 复验命令写 `.\.venv\Scripts\python.exe web\selftest.py`。
实测 **该文件不存在**：`Get-ChildItem web -Filter *.py -Recurse` 只返回
`web\app.py`；`Test-Path web\selftest.py` → `False`。
按宪法第一条「跑不出来就当它是错的」，这个闸门在订正前是**不可判定**的——
照抄它会得到 `can't open file` 而不是 PASS/FAIL。

**候选方案**：

- **A（选定）** 改为真实入口 `web\app.py --selftest`，并在 PHASE.md 注明：
  若优化轨完成 web 分层重构、真的拆出 `web/selftest.py`，届时由审查轨改回
  并附实测输出。
- B 审查轨自己创建 `web/selftest.py` 让命令成立。

**选定 A 的理由**：`web/**` 是优化轨独占领土（宪法第五条），审查轨只读。
且 `web/selftest.py` 是**重构后的目标状态**，不是当前状态——把闸门写成未来
状态，等于让闸门永远失败。闸门必须描述当下可跑的东西。

**否决 B 的原因**：直接越界写 `web/**`，重演 L-01（同一张折叠表两份拷贝对
同样字节给出相反结论）的事故模式。宪法第五条那条边界是刻意的：
它保证需求被写成文字而不是被偷偷实现掉。

## D-130a probe_contract 的严重级判据：dict 卡闸门 / array 不卡（R118a）

**问题**：`probe_contract` 发现前端把非字符串值整体塞进 `esc(...)`。第一版把
dict 与 array 一并判为卡闸门缺陷，实测发现这会误报。

**实测依据（JS 语义，不是偏好）**：`String({gan:"乙"})` === `"[object Object]"`
（信息**完全丢失**，页面出现无意义字面量）；
`String(["嫁娶","捕捉"])` === `"嫁娶,捕捉"`（逗号拼接，**可读**，只是丢了列表
结构与样式）。两者后果不同量级。

**候选方案**：

- **A（选定）** dict → 判 TYPE 卡闸门（数据显示错误，MAJOR）；
  array → 降 SOFT 进 OPTIMIZE_BACKLOG（体验瑕疵，MINOR）。
- B 一律卡闸门。
- C 一律不卡。

**选定 A 的理由**：本文件严重级定义里 MAJOR 是「数据显示错误」，MINOR 是
「体验瑕疵」。`[object Object]` 是前者（用户看不到任何真实数据），
`"嫁娶,捕捉"` 是后者（数据都在，只是没做成 pill）。按后果分级才对得上定义。

**否决 B 的原因**：会把黄历宜忌这类「显示正确但不够漂亮」的项塞进闸门，
而 PHASE.md 明写 MINOR/NIT 不得阻塞阶段推进——「这是本协作模型唯一会死锁
的地方」。实测若按 B，本轮 TYPE 会从 1 涨到 3，多出的 2 条都是可读输出。
**否决 C 的原因**：`[object Object]` 出现在排盘结果主区（五行分布、日运），
那是产品的核心输出。用户看到的是字面量而不是自己的五行——这不是瑕疵。

## D-131a probe_ui_smoke 的失败判据：只认 `.no-evidence` 内的失败文案（R118a）

**问题**：`index.html` 每个 handler 都写
`try{...}catch(ex){ r.innerHTML = '<div class="no-evidence">…失败：'+ex.message }`。
异常被 catch 吞掉，**console 是干净的**。所以「零 console.error」不足以判通过，
必须同时检查容器内容不是失败文案。但第一版在**整个容器文本**里搜「不可用」
等词，实测立刻误报：

    [FAIL] btn:bazi: 容器渲染失败文案（命中 '不可用'）…

追查发现命中的是**古籍原文本身**——`/api/bazi` 带出的引文里有
「前六時干辰入墓亦不可用六儀擊刑歌曰…」。即：正常渲染被判成失败。

**候选方案**：

- **A（选定）** 失败文案只在 `{result} .no-evidence` 元素内搜（那是全部
  handler 唯一的失败/空结果渲染出口），古籍原文在 `.ev-text` / `.hit-disc` 内，
  天然不受影响。
- B 保留全文搜，维护一张「古籍原文里合法出现的失败词」白名单。
- C 只看 console.error，不看容器内容。

**选定 A 的理由**：判据落在 **DOM 结构**上而不是词表上。前端把失败信息渲染
到哪个 class 是它自己的既定约定（全文件一致），比"哪些词算失败"稳定得多。
古籍语料有 47 部书 62,109 个单元，任何词表都会被某本书撞穿。

**否决 B 的原因**：白名单必然漏。这类"靠记住一个清单"的机制正是宪法第三条
偏离 2 的教训（分歧点必须由结构判定，不靠人记）。
**否决 C 的原因**：会让本轮 7 个 TypeError 按钮**全部误判为通过**——它们
的异常都被 catch 吞了，console 干净、容器有内容（"计算失败：…"）。C 等于
放弃这个 probe 的核心用途。这也是本 probe 唯一能自欺的地方，已封住。

## D-132a probe_ui_smoke 单用例 25s 预算 + 服务预热 + 早退（R118a）

**问题**：第一版每用例等 40s，且未预热。实测整轮跑不完（超出工具 600s 上限
被杀）。成因两条：(1) `/api/bazi` 首请求要加载 bge 语义模型（`web/app.py`
模块 docstring 明说「首请求可能较慢」），这段时间会算进第一个用例的等待；
(2) R000a-03 使 8 个面板不可见，其中的按钮 playwright 默认要等满 30s
才报 not visible，×8 就是 240s 白等。

**候选方案**：

- **A（选定）** 三件一起做：起服务后先 HTTP 预热一次 `/api/bazi`（把模型
  加载摊掉，实测 0.1s 说明缓存已在）；`set_default_timeout(4000)` 把成片
  不可见的等待从 30s 压到 4s；单用例预算降到 25s，并加早退判据——点击后
  2.5s 内既没发出任何 `/api` 请求、又已捕获未捕获异常 → 结论已定，立即判失败。
- B 只加长总超时，跑多久算多久。
- C 缩短预算到 5s。

**选定 A 的理由**：早退判据是**基于因果**而不是基于时间的——「handler 在
fetch 之前就抛了」这件事，等再久也不会变。而预热保证「慢」不会被误判成
「坏」，这是 25s 能成立的前提。实测：整轮从超时未完成变为约 3 分钟跑完，
且失败结论与 40s 版本**完全一致**（28 个失败用例逐条同名），说明缩预算没有
制造假阳性。

**否决 B 的原因**：闸门必须能在一条命令里跑完。跑一次要十几分钟的闸门，
实际上就不会被跑——那等于没有闸门。
**否决 C 的原因**：`/api/research`、`/api/compare_works` 是多轮检索 + 跨书
比对，真实耗时可达十几秒。5s 会把慢判成坏，制造假缺陷，而修复轨为假缺陷
改代码是纯损失（交接文档明确警告过这一点）。

## D-133a probe 的清理必须在 finally 里——本轮自己踩了 L-22（R118a）

**事故原文**：`probe_contract` 第一版把清理写在成功路径末尾。开发期它中途抛过
一次异常（`sqlite3.OperationalError: cannot DELETE from contentless fts5 table:
derived_fts`），清理代码没执行。事后核查 `knowledge.db`：

    derived 4  →  id=3 method='probe_contract' claim='probe_contract 契约探针占位'
                  id=4 method='probe_contract' claim='probe_contract 契约探针占位'
    favorites 2 → id=1/2 ref_id='probe_contract' title='probe_contract 占位收藏'

即：**写了「自测不得污染真实库」的 probe，自己污染了真实库**。真实基线是
derived 2（两条 2026-08-14 的真实研究结论）+ favorites 0。

**这条记下来是因为它同时踩了两条既有教训**：L-22（写端点自测不得污染真实库）
与宪法第一条（「不许偷懒走捷径……把"看起来对"当证据」）。probe 报告最后一行
明明打印了 `history 行数 43 -> 43` 看起来清理正常——但那行只覆盖 history.db，
knowledge.db 的两张表当时根本没进核查视野。**报告说清理了，不等于清理了。**

**候选方案**：

- **A（选定）** 把 seed→scan→cleanup 重构成 `try/finally`，清理函数
  自身吞异常（清理阶段再抛会掩盖真正的失败原因）并把
  `⚠ 清理未完成：<异常>` 打进清理清单，让失败可见而不是静默。
- B 清理放 `atexit`。
- C 每次跑前先扫一遍占位行删掉（自愈式）。

**选定 A 的理由**：`finally` 的作用域与"本轮写入"的生命周期完全一致，且
`created_derived` / `fav_id` 就在同一函数作用域内，无需把状态提到全局。
把清理失败**打印出来**是关键——静默的清理失败正是本次事故的成因。

**否决 B 的原因**：`atexit` 在硬崩溃（SIGKILL、解释器致命错误）时不跑，
且它与 `TestClient` 的生命周期交织，出问题更难定位。
**否决 C 的原因**：自愈会掩盖问题。若 probe 每次开局都静默删掉上轮残留，
就永远发现不了「清理没生效」这件事——而这正是本轮真正的缺陷。
按宪法第一条，机制的失败必须可见。

**复验**：清理残留后重跑，`<py> probes\probe_contract.py` → 退出码 1、
结论与重构前**完全一致**（`HARD=10 TYPE=1 SOFT=15 SKIP=0`，说明重构没改判据）；
数据库核查 `derived 2 | probe leftovers 0`、`favorites 0 | probe leftovers 0`、
`history rows 43`。`probe_ui_smoke` 的清理本来就在 `finally` 内（服务进程需要
无条件 terminate），未受此问题影响。

## D-134a 测试侧强制显示面板，以解除级联遮挡下的测量盲区（R119a）

**问题**：R000a-03（`.rtab` 无事件绑定）使 8 个 `.rsec` 面板永不可见，
面板内 7 个按钮 playwright 判 not visible **根本点不到**。这些按钮自身
是好是坏无法测量——缺陷清单残缺，修复轨要分两轮才能拿到完整信息
（第一轮修标签，第二轮才发现里面的按钮也坏）。

**候选方案**：

- **A（选定）** probe 在浏览器里用 `evaluate` 直接给目标 `.rsec` 加
  `active` class 强制显示，仅用于**让按钮可点**；标签切换用例保持原样点击、
  原样判失败；被强制的按钮用例在结论前缀显式标注
  `[面板由测试侧强制显示——标签切换本身仍是 R000a-03 缺陷]`。
- B 等修复轨修完 R000a-03 再测里面的按钮。
- C 审查轨直接在 `web/static/index.html` 补上标签切换逻辑。

**选定 A 的理由**：审查轨的产出是**信息**。级联遮挡让一次审查只能产出部分
信息，而信息越完整修复轨越少走弯路——本轮正是靠它发现了 `#worksBtn` 其实
是好的（证明修复面比看起来小）、以及 threads 的 422 还被 TypeError 掩盖着
（提醒修复轨别只看"点了有反应"）。显式前缀标注保证不会有人误读成
"标签已经好了"。

**否决 B 的原因**：串行等待浪费一整轮，且违背审查轨的职责——
把已知可测的东西留到下轮不是纪律，是拖延。
**否决 C 的原因**：越界改 `web/**`（宪法第五条）。而且这恰恰是那条边界存在
的理由：「它保证需求被写成文字而不是被偷偷实现掉」。强制显示只发生在
测试进程的浏览器 DOM 里，磁盘上的 `index.html` 一个字节没动
（`git diff -- web` 为空可验）。

## D-135a 为 `$.` 误用单独立静态闸门，与浏览器冒烟并存（R119a）

**问题**：R000a-01 的机制是 `$()` 是函数却被当对象用。`probe_ui_smoke` 能抓到
它的运行时后果，但要起服务 + 起浏览器，约 3 分钟。修复轨改 49 行代码时需要
一个秒级的自查手段，否则要么反复等 3 分钟，要么干脆不跑。

**候选方案**：

- **A（选定）** 新建 `probes/probe_dollar_misuse.py`：纯静态正则扫描，
  额外产出**按 handler 归属的处数映射**（= 修复清单）与**反向的干净 handler
  列表**。与冒烟并存，分工明确：静态证明"这个写法一个都不剩"，
  冒烟证明"点了真的有反应"。
- B 只靠冒烟，不加静态闸门。
- C 把静态检查并进 `probe_contract`。

**选定 A 的理由**：两侧是**独立见证**而非重复。实测交叉吻合——静态说只有
3 个 handler 干净（#form / #worksBtn / #newsRefresh），冒烟恰好也只有这 3 个
通过。宪法第三条偏离 4 明写「没有已知阳性对照的质量闸门等于没有闸门」，
两个独立方法得出同一结论正是这种对照。而且静态侧能给出**归属映射**，
这是冒烟给不了的——冒烟只知道"这个按钮坏了"，不知道"它有几处要改、
还有一处在 `searchByWork()` 里不属于任何按钮"。

**否决 B 的原因**：修复轨拿不到秒级自查，实际会导致"改完不验就提交"。
另外冒烟有天然盲区：`searchByWork()`（书目卡片点击跳检索）那 1 处不在任何
按钮 handler 内，逐个点按钮永远覆盖不到它。
**否决 C 的原因**：`probe_contract` 的职责是**字段名契约**（前端读的名字 vs
真实响应）。`$.` 误用是**取值写法**错误，与响应契约无关。混进去会让两个
闸门的失败含义模糊，将来 `probe_contract` 报 FAIL 时读者要先分辨是哪类问题。
一个闸门一个判据。
## D-226b R178b 决策：解读层用确定性规则引擎取代 LLM（2026-08-20）

**候选方案**

- **A（选中）** 删 `src/guji/llm_reader.py`，全部解读走 `guji.interpreter`
  规则引擎；响应字段 `llm` → `interpretation`，`use_llm` 开关整体消失。
- **B** 保留 `llm_reader`，把 `interpreter` 作为「未配置 key 时的降级路径」。
- **C** 保留 `use_llm` 开关，两个引擎并存由用户选。

**选中 A 的理由**

1. **可复验性**。宪法第一条要求任何状态断言附一条能复现它的命令。LLM 输出
   同输入不同输出，`web/selftest.py` 对它只能断言「是 str 或 None」——这不是
   断言，是形状检查。换成 interpreter 后可以断言**同输入两次输出逐字相同**
   （已加 `bazi.interpretation.deterministic`），这才是可复验。
2. **零网络零成本**，且 key 不再存在于磁盘（原 `llm_config.json` 含真实 sk- key）。
3. **G2/G7 更强**。生成式解读存在编造引文的可能（G2 存在的原因）；interpreter
   的每句话都能回指一个 calc 字段或一条引文（`basis` 字段列出来源）。

**否决 B 的理由**：两条解读路径就有两套输出契约，前端要同时处理 str 与 dict，
selftest 要覆盖两个分支——而 LLM 分支永远无法确定性断言。降级路径的价值在于
「主路径不可用时仍可用」，但这里主路径（规则引擎）恰恰是更可靠的那个，降级
方向是反的。
...
## D-227b R178b 决策：web 层四段分层 + 统一异常映射（2026-08-20）

**候选方案**

- **A（选中）** `deps / schemas / services / errors / routers / selftest` 六文件；
  services 不 import fastapi，抛业务异常，errors 统一映射状态码。
- **B** 只把 `__main__` 的 912 行自测搬出去，`app.py` 其余保持单文件。
- **C** 按 FastAPI 官方 `APIRouter` 惯例只拆 routers，业务留在路由函数里。

**选中 A 的理由**

1. `app.py` 2277 → 78 行，其中业务 0 行。
2. **异常映射收成一处**是这轮的实际收益：原先 40 余处 `HTTPException` 散落各
   端点，同一条「year 须在 1900-2100」在 bazi/taohua/hehun/qiming 里被复制四遍，
   措辞漂移正是 R000a-04 那类契约漂移的来源。现在校验只在 `schemas.py`，
   映射只在 `errors.py`。
3. services 不认识 HTTP → 每个 service 都能在没有 HTTP 栈时被直接调用与复验。

**否决 B 的理由**：自测搬走后 `app.py` 仍 1365 行，业务/校验/序列化仍然混在
路由函数里，R000a-04 的漂移土壤没动。

**否决 C 的理由**：只拆 routers 会把业务留在路由函数里，services 层缺失意味着
CLI 与 web 仍各自编排一遍 src/guji（两端输出一致性靠人工保证）。

**代价与实测**：`git show HEAD:web/app.py` 取回原文逐端点 diff（36 个端点 ×
参数表/响应键/嵌套结构/76 错误用例/41 边界值），确认无漂移；web selftest
130 → 138 checks 全 PASS，无断言丢失。

## D-228b R178b 决策：/api/tarot/draw 顶层契约钉为两键（2026-08-20）

重构中该端点一度返回 `card` + `interpretation` + **`draws`** 三键。

**候选方案**：A（选中）删 `draws`，顶层回到 `card`+`interpretation`；
B 保留 `draws` 让前端一次拿到全部牌面。

**选中 A 的理由**：`draws` 与 `/api/tarot` 的牌面是同一份数据，两个端点两处
存放，前端不知该信哪个；且这是超出「`llm`→`interpretation`」授权范围的**未登记**
响应扩张。多张牌阵本就是 `/api/tarot` 的职责（含 position 牌阵位置），
`/api/tarot/draw` 的定位是首页单张快速入口。

**否决 B 的理由**：省一次请求换来契约重复，且 `/api/tarot` 已能覆盖多张需求。

补 `tarot.draw.keys` standing 断言钉死顶层键集合——原 `tarot.draw` check 只断言
`card` 内部字段，看不见多余键。

## D-229b R178b 决策：/api/daily 的 date 改查询参数（2026-08-20）

**实测事实**：重构前 `date` 声明为 `req: DailyRequest = DailyRequest()`，即 GET
端点的**请求体**模型。实测 `GET /api/daily?date=2026-03-03` 旧实现返回今天
（查询串被完全忽略），只有往 GET 里塞 JSON body 才生效。

**候选方案**：A（选中）改为查询参数并补非法日期 400 校验；B 原样保留 body 语义。

**选中 A 的理由**：查任何日期都返回今天是 bug 不是契约——没有任何调用方能
依赖「参数被忽略」这个行为得到正确结果，前端也从未传过它。GET 端点用查询
参数是唯一合理形状。附带必须补边界校验：改成查询参数后 `?date=garbage` 会
走到计算失败降级支并往 `daily_cache` 落一条 key 为 garbage 的脏行，故在边界
即拒（400）。

**否决 B 的理由**：保留一个「参数写了不生效」的端点，等于把 bug 当契约冻结。

补 `daily.date`（指定日期须被回显）+ `err.daily.date`（非法日期 400）两条断言。
连带删除 `web/schemas.py` 的 `DailyRequest`（已成死代码）。

## D-230b R178b 决策：前端只留一套 DOM 取值风格（2026-08-20）

R000a-01 的 49 处 `$.rq.value` 不是打字手误，是**两套风格混用**的必然结果：
`$('#id')` 是函数调用、`$.id` 是属性访问，同一个符号两种用法，写的时候一定会串。

**候选方案**

- **A（选中）** 废掉 `$`，改 `el(id)` 取元素 + `val(id)`/`num(id)`/`checked(id)`
  取值；`val()` 对不存在的元素返回 `''` 而非抛异常。
- **B** 保留 `$`，把 49 处 `$.x` 逐个改成 `$('#x')`。
- **C** 给 `$` 加 Proxy，让 `$.rq` 也能工作。

**选中 A 的理由**：B 只修了这 49 处，下次写代码同样的坑还在（符号仍有两种
用法）；A 从命名上就区分了「取元素」与「取值」，`$.` 这种写法在新代码里不
存在语义。且 `val()/num()` 顺带解决了「空输入框 parseInt 得 NaN 发给后端」
的问题——`num()` 对空值返回 null，调用方决定是否发送该字段。

**否决 C 的理由**：Proxy 让错误写法「碰巧能跑」，掩盖而非消除混用；且
`$.rq.value` 与 `$('#rq').value` 并存会让后来者不知该学哪一种。

实测：Playwright 无头浏览器点遍 15 按钮 + 12 标签，console error 0
（修复前首屏即抛 TypeError）。`temp_ui_smoke.py` 为临时脚本，提交前删除；
已在台账建议审查轨固化为 `probes/probe_ui_smoke.py`（PHASE.md 闸门 3 已点名
该路径但文件尚不存在）。

## D-136a probe 判据必须跟着被测代码走，且找不到目标时报 2 不报 0（R120a）

**事故原文**：R178b 把内联 JS 搬到 `app.js`、把裸 `fetch` 换成共享 `api()`
包装器之后，两个 probe **没有报错，而是报了通过**：

    probe_dollar_misuse：`$.xxx` 误用：0 行 / 0 处   → 退出码 0
    probe_contract：1 个含 fetch 的 handler 块，0 个字段读取点 → PASS 退出码 0

前者在扫一个已经没有 JS 的 `index.html`，后者只认裸 `fetch(` 所以抽不到任何
URL。两者都是**假通过**——比 FAIL 危险得多，因为 FAIL 会有人看，
假通过没人看。这正是宪法第四条 U-08「`probe_bcv.py` 其实永远返回 0」的同族。

**候选方案**：

- **A（选定）** 三件一起做：(1) 载体改成候选列表（`app.js` 优先、
  `index.html` 回落），(2) 判据认包装器也认裸 fetch，(3) **新增退出码 2
  「无法判定」**——载体不存在、扫不到函数定义、切不出 handler 块时返回 2
  并打印原因，绝不返回 0。
- B 只改载体路径，判据不动。
- C 在 README 写一条「重构后记得同步 probe」。

**选定 A 的理由**：关键是第 (3) 条。前两条修的是这一次的具体错位，第三条修的
是**这类错误的整个形态**——以后前端再怎么改结构，probe 最坏也只会说
「我判定不了」，而不会假称通过。0 和 2 的区别就是「我查过了，没问题」和
「我没查到东西」的区别，这两句话绝不能混。

**否决 B 的原因**：下次改结构又会静默失效。
**否决 C 的原因**：靠人记得＝不可靠。宪法第一条明确「唯一可信的状态来源是
可执行脚本，不是文档里的句子」。

## D-137a `error` 是带内拒绝键，不是契约漂移（R120a）

**问题**：`probe_contract` 对 5 个端点报 HARD「前端读 `j.error` 但真实响应
没有」。若照判就是 8 条 MAJOR。

**实测核查**（不靠读代码猜，直接打请求）：

    /api/compare_works?q=雷詬飊棺雷醯 → HTTP **200** keys=['error']
    /api/bookstudy/chapter?work_id=laozi&scheme=booksec&addr1=1 → **200** ['error']

来源：`src/guji/bookstudy.py:43/49/119/127/143/167/172`、
`src/guji/research.py:275/299/301`、`web/services.py:858`。
即这些端点在**未命中/拒绝**时返回 200 + `{"error": ...}`（G7「能承认证据
不足」的实现方式），成功时该键本就缺席。前端 `if (j.error)` 是**正确**处理。

**候选方案**：

- **A（选定）** 引入 `CONDITIONAL_FIELDS`（端点 → 只在错误/降级分支出现的
  键），这类读取判 SOFT 并标注「条件存在字段，非漂移」。
- B 判 HARD，要求后端改成 4xx。
- C 判 HARD，要求前端删掉这些判断。

**选定 A 的理由**：带内拒绝是本项目**刻意的设计**——G7 要求「证据不足」是
一等公民的合法输出，不是 HTTP 错误。把它当漂移会逼出一个**错误的修复**。

**否决 B 的原因**：会撞宪法第三条——「refusal 不是错误」这条设计在
`knowledge_schema.sql:34-39` 有明确论证（「NOT NULL evidence 会让 G7 自己的
输出无法存储」）。改成 4xx 是同一个错误换个地方犯。
**否决 C 的原因**：删掉判断会让拒绝信息在页面上消失，用户看到空白而不是
「两书均无命中」——把一个正确实现改坏。

## D-138a 闸门 6 从「查一个符号」泛化为「查一类错误」（R120a）

**问题**：闸门 6 原本查 `$.xxx`。R178b 删掉了 `$`，改用 `el()`/`val()`/`num()`，
判据前提消失，probe 报 0 命中（假通过，见 D-136a）。

**候选方案**：

- **A（选定）** 自动扫出文件里所有 `function name(...)` 定义（实测 58 个），
  再查是否有任何一处 `name.属性`（排除 JS 内建对象与函数合法属性
  call/apply/bind/name/length）。判据随代码自动更新。
- B 把名字列表改成 `el|val|num` 写死。
- C 退休闸门 6，理由是「`$` 没了，这个 bug 不会再犯」。

**选定 A 的理由**：这个 bug 的**本质**不是「`$` 这个符号」，而是「同一个
标识符既是函数又被当对象用」——只要代码里有取值辅助函数，这个坑就一直在。
A 让判据自己跟着代码长，下次再改命名也不会失效。

**否决 B 的原因**：写死名字＝把 D-136a 的错误再犯一次，只是换了个符号。
**否决 C 的原因**：R000a-01 一次就制造了 11 个按钮全崩。这类「静默返回
undefined」的错误在 JS 里毫无编译期保护，代价与概率都不支持退休。
且 A 的成本只有几十行。

## D-139a 断言集只增不减，落成闸门而非靠人比对（R120a）

**问题**：宪法第二条红线第 2 项禁止「为了让数字变好而放宽闸门」。R178b 把
912 行自测重构成独立 `web/selftest.py`，断言 130 → 138。**只看数字增加不能
证明没有暗删**——完全可以删 5 条、加 13 条，数字照样涨。而人工比对 130 个
名字不现实，且属宪法第一条禁止的「凭印象」。

**候选方案**：

- **A（选定）** 新建 `probe_selftest_regress`：把自测输出末行的 **check 名字
  集合**与基线快照比对，少任何一个即 FAIL。基线存
  `probes/selftest_baseline.json`（审查轨独占 probes/，与优化轨无冲突），
  并支持 `renames` 字段登记**已审核的改名**。
- B 比对断言**条数**。
- C 比对 `web/selftest.py` 的行数或文件哈希。

**选定 A 的理由**：名字集合是**语义**标识，行数与条数都不是。删一条断言必然
少一个名字，改名也会被抓到（旧名消失）。`renames` 字段是关键设计：改名是合法
操作，但必须**显式登记**并附审查依据，否则「改名」会成为删除的掩护。

**否决 B 的原因**：条数可以靠增补掩盖删除，这正是要防的手法。
**否决 C 的原因**：行数/哈希在任何无害重构后都变，会天天误报，
最终导致闸门被忽略——一个天天红的闸门等于没有闸门。

**本轮实测**：`130 - 1 + 9 = 138` 对上。唯一消失的 `ask.llm.shape` 由
`ask.interpretation.shape` 接管，核准依据是**新断言更强**（`web/selftest.py:671-677`
断言 interpretation 是 dict、`ok is True`、engine 含「无 LLM」、sections 与
citations 均非空、且两次同输入 text 完全相等）。已登记 renames。

## D-140a 生成文本隔离用「叙述 vs 引文」二分判据，并强制阳性对照（R120a）

**问题**：宪法第三条硬约束「绝对不可入库的一类内容：生成文本」。R178b 用
确定性规则引擎取代 LLM，但解读文本仍是机器合成叙述（非印本原文），
仍须与语料物理隔离。要证明这一点，就得在语料里搜解读文本。

**第一版判据错了，实测报出 6 处「污染」**：

    /api/bazi 片段 '癸不可离也，春乙见丙，卉' → corpus.unit=1

**误报原因**：解读本来就**应该**引用古籍原文，`interpretation.text` 里混着
引文，那些片段在语料里当然搜得到。

**候选方案**：

- **A（选定）** 二分判据：只取**叙述部分**（`sections[].lines`，机器合成的话）
  去搜，要求零命中；**引文部分**（`citations`）单独做**阳性对照**，
  要求能在语料里找到。
- B 整段 text 去搜，维护一张「允许命中的引文」白名单。
- C 只做静态检查（interpreter.py 不 import 数据库），不做动态搜索。

**选定 A 的理由**：这个二分正好对应宪法第三条的判据本身——「能否追溯到
印本/底本」。叙述追溯不到，必须零命中；引文追溯得到，必须命中。
**同一个 probe 同时验证了「没污染」和「引用真实」两件事。**

**否决 B 的原因**：白名单必然漏，且引文集合随每次检索变化，无法维护。
**否决 C 的原因**：静态只能证明「这个版本的代码没写入」，证明不了
「实际跑一遍没进去」。宪法第三条偏离 4 要求独立见证。

**阳性对照救了整条结论**：改成 A 之后第一次跑，对照 **0/6 命中**。
追查发现我把针 `re.sub(r"\s+","",text)` 去了空白，而库里存的原文**保留换行**
（实测 `raw[:12]` 命中 1 条、`stripped[:12]` 命中 0 条），针永远匹配不上
干草堆。保留原始空白后 **6/6 命中**。
**没有这个阳性对照，「叙述零命中」会被当成隔离成立，而真相是搜索根本没生效。**
这就是「没有已知阳性对照的质量闸门等于没有闸门」的一次现场演示。

## D-141a 审查轨工具的上游依赖需要专门闸门；且该闸门必须纯静态（R120a）

**问题**：R178b 删除 `src/guji/llm_reader.py` 后，`scripts/ask_bazi.py --llm`
实测崩溃：退出码 1、`ImportError: cannot import name 'llm_reader' from 'guji'`。
这类缺陷的讨厌之处：它在**审查轨自己的领土**（`scripts/`），优化轨没有义务
发现；而 13 道闸门只跑其中 9 个脚本，`ask.py`/`ask_bazi.py`/`research_thread.py`
零覆盖，可以坏很久没人知道。

**修复本身**：改走 `guji.interpreter` 确定性引擎；`--llm` 保留为 `--interpret`
的别名（`dest="interpret"`），旧命令行不破。实测退出码 0，两次运行输出
**逐字节相同**（4297 chars）。

**闸门设计的候选方案**：

- **A（选定）** 纯静态 AST 扫描：找出 `scripts/` 与 `probes/` 里所有对 `guji`
  的引用（**含函数体内的延迟导入**），逐个核对 `guji` 是否还有那个名字。
- B 真导入每个模块（`importlib` + `exec_module`），看是否抛异常。
- C 真跑每个脚本的 `--help`。

**选定 A 的理由**：B 是我的第一版，**实测是错的**——我以为「不调 `main()`
就没有副作用」，但模块级代码在 `exec_module` 时就会执行，而 `probes/` 里
不少模块在模块级就连数据库、跑查询、打表格。实测整个 probe 跑到工具 600s
上限被杀，还打了一屏无关输出。更关键的是：**B 恰好抓不到本轮这个 bug**——
`from guji import llm_reader` 写在函数体里，只在 `--llm` 时才执行，
真导入根本走不到那行。A 零副作用、秒级、且天然覆盖延迟导入。

**否决 C 的原因**：`--help` 由 argparse 处理，在任何延迟导入之前就返回，
覆盖面比 A 还窄。

**已知局限（诚实声明）**：静态分析只核对「名字还在不在」，不核对函数**签名**
是否变了。签名级回归由各闸门自身的实测覆盖，本 probe 不声称覆盖。

**排除项**：`probes/probe_coverage.py` 与 `probes/probe_show.py` 带 UTF-8 BOM
导致 `ast.parse` 报 `invalid non-printable character U+FEFF`，自 initial commit
（83d7604，2026-08-14）起即如此，**与 R178b 无关**。列入 `KNOWN_BAD` 排除
而不是「顺手修掉」——本闸门的职责是抓「上游改动打断审查轨工具」，
混入无关历史脏数据会让它的失败含义模糊。清理它们应当作独立任务。

## D-142a probe 自身的副作用同样受宪法第五条约束（R120a）

**事故**：`probe_scripts_importable` 第一版用 `exec_module` 真导入
`scripts/` 与 `probes/` 下 88 个模块，意图是"导入级冒烟"。实测后果：

1. 超时——跑到工具 600s 上限被杀（`probes/` 里多个模块在模块级连数据库、
   跑全量查询、打表格）。
2. **改了共享状态**——5 个 `data/catalog/*.json` 被重新生成，
   `git diff --stat` 显示 `50 insertions(+), 676 deletions(-)`；
   另把 KR1a0001/0006/0007 原始语料拷进 `probes/data/raw/`（130 文件）、
   解压了一个 vendor zip。

第 2 条是真正严重的：`data/catalog/*.json` 属两轨共享状态，
审查轨对它没有写权。676 行删除若随本轮提交，就是「同一份清单存在两份拷贝、
对同样字节给出不同结论」——`LESSONS.md` L-01 的原样重演。

**候选方案**：

- **A（选定）** 改为纯静态 AST 分析（零执行）；并把这条教训写成纪律：
  审查轨新增 probe 时，**默认假设它有副作用**，除非它只做解析与读取。
- B 保留真导入，但在 probe 里做沙箱（临时目录 + 猴补丁挡住写操作）。
- C 保留真导入，跑完用 `git checkout` 还原。

**选定 A 的理由**：A 同时解决了三件事——零副作用、秒级、且**恰好**能抓到
本轮那个 bug（延迟导入，真导入反而走不到）。这不是妥协，是更好的方案。

**否决 B 的原因**：沙箱要拦住 sqlite 写、文件写、网络、子进程，
每一处漏了都是一次 L-01。用几百行不可靠的拦截换一个本来不需要的能力，
不划算。
**否决 C 的原因**：`git checkout` 还原**只能还原被跟踪文件**。本次事故里
`probes/data/`（130 个未跟踪文件）与解压残留就不在其中，需要手工删。
而且「先破坏再还原」在双轨并行下有窗口期——修复轨此刻可能正在 merge。

**处置与复验**：还原 5 个 manifest 与 zip、删除 `probes/data/` 与解压残留，
随后**重跑 13 道闸门**确认无回归（退出码全 0）、`git status -- data/` 干净。

## D-143a 先建基线量尺，再写可测量的 spec（R121a）

**问题**：spec 准入门槛要求「可自动测量」。但「首屏更快」「点击区更大」
这类话本身不可测——缺的是**当前值**。没有当前值，事后既无法判定达成、
也无法判定退步（宪法第一条：任何状态断言必须附能复现它的命令）。

**候选方案**：

- **A（选定）** 先写 `probe_ui_baseline.py` 实测出全部当前值，
  再写 spec，spec 里每条判据都写成「当前 X → 目标 Y」并附同一条复验命令。
  该 probe 明确定位为**量尺不是闸门**：退出码 0 只表示"测完了"，不做判定。
- B 直接写 spec，目标值凭经验填（如「点击区 ≥44px」），验收时再测。
- C 用 Lighthouse 之类现成工具出报告。

**选定 A 的理由**：B 的隐患是**目标值可能已经达标或根本不可达**，而写的时候
不知道。实测证明这个担心是真的——首屏 FCP 375px 已经是 128ms、横向溢出已经
是 0、reduced-motion 已经生效。若按 B 写成「改进项」，优化轨会去"改进"
四个本来就达标的指标，纯浪费；而真正的大头（59 个过小点击目标、31 处对比度
不足）反而可能被漏掉。**先测再写，直接改变了 spec 的内容。**

把量尺与闸门分开也是刻意的：量尺天天变（每次改版都变），若给它加通过/失败
判定，它会变成一个天天红的闸门，最终被忽略。判定留给 spec 的验收表。

**否决 B 的原因**：见上，实测已证伪。
**否决 C 的原因**：Lighthouse 需要新依赖（撞宪法第二条红线第 3 项），
且它测不了本项目特有的东西——比如「每条古籍引文是否带非空出处」
（判据第 13 条），那是本项目的立身之本，任何通用工具都不会测。
自建 probe 用的是已在 venv 里的 playwright，零新依赖。

## D-144a spec 里「可回滚」不是附加项，而是自主审美决策的前提（R121a）

**问题**：本协作模型授予审查轨「审美最终决定权 + 不询问即执行」。
这在正常情况下是效率优势，但它有一个隐含风险：**审美是主观的，
用户事后可能不认可，而代码已经改完了。**

**候选方案**：

- **A（选定）** 把「视觉方案可一键切回旧样式」写成 User Story 5（P1）与
  Success Criteria 第 12 条，并要求**两套视觉各自**满足全部其他判据
  （对比度、点击目标、35/35 用例、13 闸门）。即：回滚不是"留个后门"，
  而是"两套都是一等公民"。
- B 不写回滚，靠 git revert 兜底。
- C 写回滚，但只要求"能切回去"，不要求旧样式也满足新判据。

**选定 A 的理由**：这条是**其余所有审美要求得以自主执行的前提**。有了它，
审查轨选错方向的代价是"用户点一下切回去"，而不是"重做一轮"。
所以它必须是 P1，且必须可测——不可测的回滚等于没有回滚。

要求"两套各自达标"是关键细节：若旧样式在切回后功能不全或对比度崩了，
那个回滚开关就是装饰品，用户实际上不敢用。

**否决 B 的原因**：`git revert` 是**开发者**的能力，不是**用户**的能力。
用户在浏览器里说"这个配色我不喜欢"，不该需要一次提交回滚和一次重新部署。
...
## D-145a probe 不得把对方的内部命名钉死成契约（R122a）

**事故**：读书三子标签的 `data-rsec2` 值，两轨从相反方向各改一次，仍然对不上：

    R178b：HTML  bs-structure → bsStructure（驼峰）
    R120a：probe 选择器跟着改成驼峰，实测 35/35 PASS
    R179b：HTML  bsStructure → bs-structure（改回 kebab），
           理由「那三个值是验收契约不是内部命名」，还加了注释叮嘱别改驼峰
    R122a：probe 首跑 6 个 TimeoutError

产品侧从头到尾都是对的（`app.js:1361 BSSEC_PANELS` 与 HTML 始终自洽）。
错的是**我的 probe 把对方的内部命名当成了契约**，于是每次对方改命名，
要么 probe 失效（假缺陷），要么对方被迫为迁就 probe 而回滚一次无害重构。
后者更糟——它让验收工具变成了重构的阻力。

**候选方案**：

- **A（选定）** 子标签值**运行时从 DOM 发现**（`querySelectorAll('.rtab[data-rsec2]')`
  取 `dataset.rsec2`），probe 只断言"有三个子标签、点了能 active、
  对应面板出内容"。用例表里用 `@subtab:N` 占位符按序号解析。
  稳定契约改为**面板容器 id**（`#bsStructure`）——那是渲染目标，
  前端拿它往里填内容，改动它一定会同时改 JS，不会单方面漂移。
  另加一条 `subtab.discovery` 用例：子标签数量变化时直接报出来。
- B 维持写死，并在 `docs/` 里写一条「data-rsec2 的值是验收契约，不得修改」。
- C 维持写死，每次对方改名我就跟着改。

**选定 A 的理由**：A 让这类事故**不可能再发生**——probe 不再对命名有意见。
而 B 和 C 都建立在"两轨记得对齐"之上，本轮实测证明这个假设不成立：
双方都读了对方的代码、都留了注释、都是善意的，**仍然对不上**。
异步协作里"靠记得"就是不可靠。

另一个理由是职责边界：命名属于实现细节，是优化轨的领土。审查轨该断言的是
**行为**（点了标签，对应面板出现内容），不是**名字**。R179b 为了迁就我的
probe 回滚了一次无害的命名统一，那说明我的判据越界了——它把对方的重构
自由给限制掉了，而闸门的职责是防缺陷，不是防重构。

**否决 B 的原因**：把实现细节升格成跨轨契约，会不断累积这类"不许动"的
清单，最终让重构举步维艰。真正该进契约的是容器 id 与可见行为。
**否决 C 的原因**：每轮手工追命名，且每次追错都产生一批假缺陷——
修复轨会为假缺陷改代码，那是纯损失。

**复验**：改后 `probe_ui_smoke` **36/36 PASS 退出码 0**；
`subtab.discovery` 实测输出 `发现 3 个子标签 ['bs-structure', 'bs-chapter',
'bs-summary']（期望 3 个）`——无论将来叫什么，它都会照实报出来。

## D-146a 若将来做 BYOK AI 解读，必须同时满足的七条约束（R123a，应优化轨 R180b 请求预先落定）

**背景**：优化轨 R180b 提案 P4「BYOK AI 深度解读」，明确不默认做、待用户拍板，
并请审查轨**预先**把约束写成决策，使用户拍板时即有现成边界、不需临时讨论。
这个请求是对的——临时讨论红线是红线最容易被稀释的时刻。

**审查轨立场：不收编进 `specs/004`，但边界先立好。** 理由是 LLM 移除是
R178b 的核心决策（成本 + 不可复现），`specs/003` Out of Scope 也明确排除。
在用户拍板之前它不是需求，写进 spec 会变成"待办"，那是错误的信号。

**若用户将来拍板要做，须同时满足以下七条，缺一即不得实施**：

1. **仅 BYOK，服务端零 key**。用户自己的 key 只存浏览器本地，
   服务端不接收、不转发、不记录。可验证判据：服务端代码零 key 字段、
   零出站请求；抓包无 key 经过服务端。
2. **输出永不入库、不参与检索**。与现有解读文本同等待遇之上再加一层：
   `history.db` 也不写（现有确定性解读可写 history 是因为它可复现；
   AI 输出不可复现，写进历史等于把不可复现的东西变成看似有据的记录）。
   判据：现有 `probe_no_generated_in_corpus` 扩展覆盖，corpus/knowledge/history
   三库全部零命中。
3. **永久且显著标注「AI 娱乐解读」**，与古籍引文区在视觉上不可混淆
   （承接 003 US6 三类内容可辨）。判据：DOM 断言标注存在且与引文区不同容器。
4. **不可复现性必须显式告知**。同输入不同输出是 LLM 的本性，
   而本项目全部其他输出都可复现。UI 必须说明这一段"再点一次可能不一样"——
   否则用户会把它当成和坐标一样可靠的结论。
5. **确定性解读不得被替代或降级**。AI 是**附加**层，关掉它页面功能完整；
   判据：关闭 AI 后 UI smoke 与 selftest 全绿、结果区内容非空。
6. **不得为 AI 输出提供任何"引用"外观**。不得让模型生成引文、书名、页码、
   卦爻地址。判据：AI 区域内零 citation 结构；`eval_g7.py` 的 FABRICATIONS
   必须保持 0。
7. **默认关闭，用户显式开启**。首次开启需一次性确认（说明它会把数据发往
   用户自选的第三方端点）。判据：默认配置下零出站请求。

**为什么把第 2 条写得比提案更严**：提案说"不落任何 db"，我确认这一点并明确
到 `history.db` 也不行。现有 `history.db` 存确定性解读是 D-039 授权的，
其正当性建立在"可复现"上——把不可复现的生成文本混进同一张表，
会让整张表的可信度降级，且未来无法区分哪条记录能复现。

**用户拍板时的最小动作**：把本条七款作为验收清单，逐条给复验命令。
若某条做不到，正确结论是"这个方案错了"，照 R-02 先例记 REJECTED 换方案，
不是放宽这七条（宪法第二条红线第 2 项）。

## D-147a MBTI×塔罗 REJECTED：语料零落点与立身之本冲突（R123a）

**问题**：优化轨提案 P3「MBTI × 塔罗本命牌」，理由是它是最强分享物，
且竞品 `adds9810/your-tarot-mbti` 已验证产品形态。

**实测**：在 `corpus.db` 全文检索 `MBTI`、`人格类型`、`十六型`，**零命中**。

对照同一次检索的其他项：

    河图数「天一生水」 11 条命中、「天五生土」 3 条
    五色                74 条命中
    十二宫（白羊/金牛/巨蟹/獅子/天秤/雙魚…）《星學大成》完整十二宫表
    MBTI 相关           ** 零命中 **

**候选方案**：

- **A（选定）** REJECTED。不收编进 `specs/004`，理由与实测数据写入 spec 与台账。
- B 收编，但要求与古籍物理隔离、独立免责声明（提案对星座的原方案）。
- C 收编，不设约束。

**选定 A 的理由**：宪法第三条的判据是「能否追溯到印本/底本」。MBTI 是 20 世纪
心理学量表，与这条判据**没有任何接口**——不是"暂时找不到出处"，是原理上不存在。

关键在于**机会成本**：同一轮实测刚刚确认十二宫是《星學大成》的真实内容
（见 D-148a），这让本项目的星座日运可以挂真实引文，形成对竞品的结构性差异。
在这个基础上再加一个纯粹无出处的 MBTI 模块，等于自己把刚建立的差异化抵消掉——
用户会合理推断"既然 MBTI 是编的，那星座和幸运色大概也是编的"。
**一个以"有出处"立身的产品，最贵的资产是"每一项都有出处"这个整体印象。**

**否决 B 的原因**：隔离能防污染，但防不了印象损失。而且 B 的成本不低
（独立路由、独立 UI 分区、独立免责），换来的是一个削弱主张的模块。
**否决 C 的原因**：会让 `probe_no_generated_in_corpus` 这类闸门的意义模糊化。

**若用户明确要 MBTI**：正确形态是**独立产品**，不挂在"知命"的古籍语料旁边。
那时它不受本项目宪法第三条约束，也不会拖累本项目的可信度。

## D-148a 十二宫是本项目语料内容，不是外来娱乐拼贴——推翻提案定性（R123a）

**提案原判断**（R180b §4 P2）：星座属"西方占星为娱乐模块，非本项目古籍语料"，
要求"独立 API 路由、UI 分区、独立免责声明——不混排，不进 corpus"。

**实测推翻**。在 `corpus.db` 检索十二宫名：

    白羊 49（KR3g0041×40）　金牛 52（×47）　巨蟹 39（×36）
    獅子 45（×35）　天秤 30（×28）　雙魚 50（×42）
    同一单元内最多同时出现 10 个宫名 → KR3g0041 @KR3g0041_WYG_015-29b

`KR3g0041` = **《星學大成》，命理类，31 文件 472,894 字**，
source_url 为 kanripo、zip_sha256 与 fetched_at 齐备（provenance 完整）。
真实原文 `@KR3g0041_WYG_001-1a`：

> 定十二宫神二十八宿分野所屬　角亢秤宫辰屬鄭　氐房心卯蝎宋遊　尾箕人馬燕寅位
> 斗牛磨蝎丑吳流　女虚危瓶齊子地　室壁雙魚亥衛州　奎婁白羊魯國戌　胃昴畢酉趙金牛
> 觜參隂陽申晉地　井鬼巨蟹未秦州　柳星張周獅子午　翼軫雙女楚已

十二宫与**二十八宿分野**同表并列，属七政四餘 / 星學体系——这是中国传统星命学
的固有内容，不是外来拼贴。

**候选方案**：

- **A（选定）** 推翻隔离要求。`specs/004` 明确**不要求**十二宫与古籍隔离，
  **反而要求**它必须带真实语料引文与出处（判据第 10 条）。
- B 沿用提案的隔离方案（更保守，看似更安全）。

**选定 A 的理由**：B 表面上"更保守"，实际是**把自己的资产当成负债处理**。
竞品的星座日运只能拍脑袋写文案，我们可以挂《星學大成》的页锚点引文——
这正是「有出处的玄学」这个主张最好的一次落地。按 B 做，等于主动放弃它，
还要额外付隔离的实现成本。

宪法第三条真正禁止的是**生成文本入库**，不是"某个主题不许碰"。
判据从来是「能否追溯到印本/底本」——十二宫能，所以它是合法内容。
...
## D-149a 探针要查「是不是人话」，不只查「有没有内容」（R124a）

**事故**：`probe_ui_smoke` 在 R124a 首跑 **36/36 全绿**，而真浏览器默认模式的
排盘结果区首屏赫然显示：
...
**为什么漏**：`btn:*` 用例的判据是「容器非空 + 不停在占位文案 + `.no-evidence`
里没有失败文案 + 无 `[object Object]` + 无 console 错误」。
这套判据回答的是**「功能有没有坏」**，回答不了**「输出是不是给人看的」**。
REPAIR 阶段这样够用——那时的问题是按钮点了没反应。进了 OPTIMIZE，
用户要的是「看得懂」，判据没跟着升级。

`[object Object]` 我查了，`ten_gods` 我没查。这两者对用户是同一件事：
**屏幕上出现了程序内部的东西**。我当初只把前者写成判据，是因为它是那一轮
实测撞到的形态——把"这次遇到的具体形态"当成了"这一类问题"。

**候选方案**：

- **A（选定）** 给 smoke 加 `INTERNAL_KEYS` 断言（18 个内部键名清单），
  **只在 warm 模式判**，专业模式豁免（`specs/004` 判据 9 要求它逐字节不变，
  在那里报缺陷会逼出错误的修复）。另加 `voice.default_mode` 用例，
  把"首次打开是哪个模式"记成可见事实。
- B 只在 `specs/003` 判据表里写「不得暴露内部字段名」，靠人工看。
- C 用更宽的规则：结果区不得出现 `[a-z_]{4,}` 这类下划线小写词。

**选定 A 的理由**：清单虽然要维护，但**判据明确、误报为零**。而这批键名来自
后端响应，本来就是有限且稳定的集合——它们变化时通常伴随契约变更，
`probe_contract`（191 个读取点）会同时报出来，两个闸门互为提醒。

「只在 warm 模式判」是这条决策最关键的细节：`specs/003` 判据 14（不暴露字段名）
与 `specs/004` 判据 9（专业模式逐字节不变）在这里**直接冲突**。
若不分模式，探针会要求修改专业模式，而那会破坏判据 9 的基线——
两条判据都对，交叉点上必须显式选择作用域。

**否决 B 的原因**：宪法第一条——「唯一可信的状态来源是可执行脚本，
不是文档里的句子」。本条事故本身就是"写在 spec 里但没有探针"的直接后果：
判据 14 从 `specs/003` 起就存在，我却一直没给它写断言。
**否决 C 的原因**：会误报。古籍原文、work_id（如 `KR1a0001`）、
文件名（`mingli-tanyuan_001.txt`）都合法含下划线小写串。
宽规则的假阳性会让修复轨为不存在的缺陷改代码，那是纯损失。

**复验**：加断言后复跑 `37 个用例 PASS 36 / FAIL 1` 退出码 1，
实测报出 `['ten_gods', 'five_elements', 'day_luck', 'relations',
'day_ganzhi', 'day_master_rel']`——即它现在确实能抓到 R124a-01。

## D-150a 判据不能建立在会随数据量漂移的量上（R125a）

**问题**：`specs/003` 判据 1/2 我在 R121a 写的是「375px 点击目标 <44px：
59 个 → 0」「1280px：56 个 → 0」。R125a 复测得 **60 / 72**，
而这方向的代码并没有改动。

逐元素分类后原因清楚：

    固定 UI      1280px 16、375px 16   ← 输入框/下拉/#dailyMore/#newsRefresh
    数据驱动     1280px 56、375px 44   ← 历史行「查看/删除」各 20 个
                                          + 外部新闻链接 16 / 4 条

历史行数量随 `history.db` 增长，新闻条数随当日抓取结果变化。
**同一份代码，今天测 56、明天测 72。** 判据会在没人改代码的情况下自己变红
或变绿——那时它既不能证明改进，也不能证明退步。

**候选方案**：

- **A（选定）** 判据只数**固定 UI**（16 → 0）；在 `probe_ui_baseline` 里把
  「固定 UI / 数据驱动」两类自动分开统计并分别输出。数据驱动部分的点击区
  同样要修，但归入 US1 场景 3 的定性要求，不进可数判据。
- B 判据改成「小于 44px 的**比例**不超过 X%」。
- C 测量前先清空 `history.db` 与新闻缓存，让基数固定。

**选定 A 的理由**：A 把判据钉在**代码决定**的量上。固定 UI 的数量只随
`index.html` 的控件增减而变，那正是我要判的东西；实测两个视口都稳定为 16，
可复验。而数据驱动那部分的正确判法是「模板里的按钮尺寸对不对」——那是一个
模板问题，不是 40 个独立问题，数它的个数本身就没有意义。

**否决 B 的原因**：比例同样漂移（分母在变），而且更难解释——「12% 的点击目标
过小」无法告诉修复轨该改哪里。
**否决 C 的原因**：清空真实库违反 L-22 的精神（自测不得污染/破坏真实库），
而且新闻是外部抓取，清不掉。更重要的是：**为了让测量稳定而去改被测环境，
方向就错了**——该改的是测量口径。

**这条同时是自我纠错的记录**（宪法第一条：发现文档与实测不符时改文档并写明
此前结论已被推翻）。R121a 那两个数字是真实测出来的，但我当时没检查它们
**是否可重复**。一个数字"测出来了"和"可以当判据"是两件事——
判据必须在被测对象不变时保持不变。这一点我在 R121a 漏了。

## D-151a html2canvas 判 REJECTED——零依赖 Canvas 实测可行，它只是便利不是必需（R126a）

**先纠正我自己上一轮的错误。** R125a 我对修复轨说「`html2canvas` 需要用户明确
授权」，把决定权推回给了用户。这违反本协作模型的纪律第 3 条：
**撞红线时不要问用户，评估后标 BLOCKED/REJECTED 附实测数据，然后做下一件事。**
「等用户授权」和「停下来问」是同一件事的两种说法。本条即补上该做的评估。

**事实核查**（先查再判，宪法第一条）：

- `html2canvas` 不在仓库内（全库 `*html2canvas*` 零命中），`vendor/` 目录
  **在 audit 侧根本不存在**（`Test-Path vendor` → False）。故它属**新引入外部
  依赖**，正撞宪法第二条红线第 3 项。这一点提案自己也承认了。
- 提案的论证是「唯一新依赖 + MIT + 单文件 ~48KB + 按 §5 记 provenance +
  无 CDN 运行时零联网」。这些都对，但都在回答"引入它是否安全"，
  **没有回答"是否必需"**。红线管的是后者。

**关键实测：零依赖方案可行。** 我用浏览器原生 Canvas 2D 直接画了一张海报，
喂的是真实 `warm.energy_card` 数据（1998-07-20 14 时女命）：
...
一次 `evaluate` 即完成，含标题、能量卡全字段、幸运色色块可视化、
**出处三条**、「仅供娱乐」水印。即 `specs/004` US5 的五条 Acceptance
零依赖全部可满足。

**候选方案**：

- **A（选定）REJECTED，改用原生 Canvas 2D 绘制。** 数据本来就是结构化的
  （element / numbers / colors / hours / keywords / basis），直接画比截 DOM 更可控。
- B 引入 `html2canvas`，记 provenance 后自行使用。
- C 记 BLOCKED，M3 整体挂起等用户拍板。
- D 用 SVG 拼字符串 + `<img>` 转 canvas 导出。

**选定 A 的理由**：

1. **红线的处置规则是"换方案"，而 A 就是那个方案。** 红线第 3 项的正确结论是
   「这个任务的当前方案错了」（宪法第二条原文）。实测证明确实错了——
   它把一个不需要依赖的需求写成了需要依赖。
2. **截 DOM 本身是错的技术选择。** `html2canvas` 的工作原理是把 DOM 重新解析
   成 canvas，它对 CSS 支持不完整（渐变、混合模式、部分字体常出偏差），
   而我们要的是**固定版式的分享图**，不是网页快照。海报版式与页面版式本就
   不必相同（页面是响应式的，海报是 3:4 固定的）。用原生 Canvas 画，
   版式完全受控，且不受 003 的响应式改动影响——**这一点很重要：
   否则每次改 CSS 都可能让海报变形，而海报是要发小红书的。**
3. **确定性更好。** 原生绘制同输入必得同输出，可写成判据；
   截 DOM 的结果依赖当时的字体加载、滚动位置、动画帧，
   与 004 全篇「同输入必同输出」的纪律不合。
4. **零依赖树、零许可证义务、零 provenance 维护成本。** 48KB 不大，
   但依赖的成本从来不是体积，是"它以后会不会变、要不要跟版本、
   出问题算谁的"。

**否决 B 的原因**：撞红线且**不必要**。若它是唯一可行路径，我会记 BLOCKED
让用户拍板；但实测有等效且更好的路径，那就不该把红线消耗在这上面。
红线的额度应该留给真正无法绕开的需求。
**否决 C 的原因**：会无谓阻塞 M3。BLOCKED 是给"没有替代方案"的情形用的，
这里有。挂起一个能做的任务不是纪律，是拖延。
**否决 D 的原因**：SVG → canvas 在 Chromium 下受 `<foreignObject>` 与字体
内联的限制，实测复杂度反而高于直接绘制，且外部字体会污染确定性。

**已写入 `specs/004-warm-voice/spec.md` US5**：把"须用户授权"改为
"须用零依赖方案"，并附本条实测数据。修复轨据此可直接施工 M3，不必等任何人。

**用户仍可推翻**：若用户明确说"就用 html2canvas"，那是授权，照办；
但**默认不做**，且不需要为此停下等待。
## D-237b R183b 决策：无障碍配色按角色拆令牌，而非全局调暗

**问题**：审查轨实测 31 处文字对比度低于 WCAG AA，最差 3.18:1（每日运势的
「宜」结论——用户最想看的一句话最难读）。

**根因诊断**（这一步决定了修法）：不是"某几处颜色挑错了"。查全表后发现
31 处里 28 处指向同一个令牌 `--primary #B8860B`，而它同时承担两种**冲突**的
角色：作文字色时需要足够深才能压过浅底（4.5:1），作白字按钮背景时也需要
足够深才能托住白字（4.5:1）。两个需求都指向"更深"，而 3.25:1 两头都不够。

**候选方案**

- **A（选中）** 按角色拆令牌：`--primary`（仅装饰）/ `--primary-ink`（文字）/
  `--primary-bg`（白字背景），其余同理。值用 WCAG 公式求解。
- **B** 把 `--primary` 全局调暗到同时满足两种用法的最深值。
- **C** 逐处打补丁：31 个位置各自指定颜色。
- **D** 放宽判据到 AA 的大字号档（3:1）。

**选中 A 的理由**

1. 求解脚本实测：若按 B 求"同时满足全部背景 + 白字在其上"的单值，得
   `#7F5C08`——但它作按钮背景时白字虽过、**装饰性边框和阴影也一起变暗**，
   金箔质感丢失。角色拆分让装饰保留 `#B8860B`、只有承载文本的地方变深。
2. C 会产生 31 处硬编码颜色，下次新增组件必然漏，且判据 3 会反复回归。
   令牌是唯一"改一处即全站生效"的层级。
3. A 顺带解决了判据 12：回滚只需覆盖 6 个令牌值，不必反向修改任何规则集。

**否决 D 的理由**：这是宪法第二条红线第 2 项——「为了让数字变好而放宽验收
闸门」。AA 对 14px 正文的要求就是 4.5:1，把它按大字号档算是伪造达标。
按 R-02 先例，若某任务只能靠放宽闸门完成，那是方案错了。

**渐变按钮的单独处置**：`.daily-more` 与 `button` 用
`linear-gradient(135deg,#B8860B,#FFE66D)` 配白字，实测 1:1。求解确认
**亮端 `#FFE66D` 上白字 1.25:1，数学上不可能达标**——任何"保留原渐变"的
方案都不成立。故改深金渐变 `#8A6408→#6E5006`（白字 6.51:1）并补
`background-color` 实色兜底。

补实色兜底的理由需要说清，否则像是在讨好探针：审查轨的 probe 读
`getComputedStyle().backgroundColor`，对渐变返回 `transparent`，于是算出
1:1——这是探针盲区。但实色兜底本身是**渐变的标准写法**（渐变不渲染时它就是
实际生效颜色，也是高对比度模式与打印的取值来源）。我补它同时解决了真实
问题（白字确实不够对比）与可测量性，而不是只让数字变好。这条区别很重要：
如果只为过探针而改探针读得到的属性、真实渲染仍不达标，那就是放宽闸门。

**代价**：金色整体变深，视觉上不如原来"亮"。这是审美取舍，故判据 12 的
一键回滚是它的前提——用户不认可可以一键切回 `原版`，`legacy` 主题保留
原全部令牌值（实测该主题下对比度回到 36 处不足，正是它作为对照的意义）。

## D-152a 回滚目标不该被新判据约束——订正我自己写的矛盾（R127a）

**问题**：`specs/003` 判据 12 我在 R121a 写的是「视觉方案可一键回滚 →
具备，**且两套各自满足 1–11**」。R127a 实测暴露它与 US5 场景 3 冲突：

    新配色（默认）：对比度 <AA 0 处、固定点击目标 <44px 0 个
    旧配色（legacy）：对比度 <AA 29 处、固定点击目标 <44px 17 个

US5 场景 3 原文写的是「对比度、点击目标、横向滚动、长任务四项指标各自达标
（新样式按新目标，**旧样式按其原有水平**）」——那句是对的。
判据表第 12 条却写成「两套各自满足 1–11」，两处自相矛盾。

**为什么旧配色那 29 处不是缺陷**：回滚的定义就是「回到 REPAIR 阶段结束时的
视觉」，而那个视觉实测本来就有 31 处低于 AA（R121a 基线）。
**要求回滚目标也满足新判据，等于要求它不再是回滚目标**——逻辑上不可能同时成立。
若真按原判据执行，唯一"通过"方式是把旧配色也改成达标的，那时它就不再是
「切回旧样式」，判据 12 的整个用途（用户不认可新审美时零成本撤回）就消失了。

**候选方案**：

- **A（选定）** 判据 1/2/3（点击目标、对比度）**只约束新配色**；
  旧配色只需满足与视觉无关的几条：4（横向滚动 0）、5（长任务 0）、
  9（浏览器用例全过）、10（零契约漂移）、11（13 闸门全绿）。
- B 要求两套都达标。
- C 删掉判据 12，不要回滚能力。

**选定 A 的理由**：A 把「回滚」与「改进」分开度量。回滚要保的是**功能与数据
正确性**（切回去不能坏、不能丢数据），不是**视觉质量**——视觉质量差正是它被
替换的原因。分开之后两条判据都能成立且都有意义。

**否决 B 的原因**：见上，会消灭判据 12 的用途。这也是一次提醒：
**判据之间会互相矛盾，而矛盾往往在实测时才暴露**——我写的时候两条都觉得对。
**否决 C 的原因**：判据 12 是「审查轨自主决定审美方向」得以成立的唯一前提
（见 D-144a）。删掉它，我的审美决策就变成不可逆的赌注。

**这条是自我纠错记录**（宪法第一条：发现文档与实测不符时改文档并写明此前口径
已被推翻）。矛盾由我引入，修复轨的实现实测正确——它做出了可用的回滚
（「清晰 / 原版」两个按钮 + `data-theme=legacy`），我不该因为自己的判据写错
而记它缺陷。

## D-153a 判据要测「屏幕」，不是测「响应对象」（R128a）

**事故**：`specs/004` 判据 1/2/3 全部通过，`probe_ui_smoke` 37 用例全绿，
`web/selftest.py` 149 checks 全绿——而用户的实际反馈是
「全文都只有引经据典，看不懂也不想看」。

实测原因：那些判据测的都是**响应对象**——

    判据 2「一句话 ≤20 字」  → 测 warm.one_liner 的 len()，实测 11 字 ✅
    判据 3「首屏术语 ≤3」    → 测 warm 里几个键的文本，实测 2 个 ✅

**没有一条测它渲染在第几屏。** 实测那句 11 字的人话在 `y=92,794px`，
第 115 屏。用户要滚 91,982px 才能看到。

**这是 D-149a 的第二次发作。** D-149a 的教训是「探针要查是不是人话，
不只查有没有内容」；本条的教训是「判据要查用户能不能看见，不只查数据里有没有」。
两次都是同一个模式：**我把「数据结构正确」当成了「用户体验正确」。**

**候选方案**：

- **A（选定）** 新建 `probes/probe_first_screen.py`，判据全部落在**渲染后的
  像素坐标**上：一句话结论的 y 坐标、结果区总屏数、默认可见最长文本块字数、
  古籍占版面比例。并用 `textContent` 抽查折叠后原文与出处是否仍可取到。
- B 在 `probe_ui_smoke` 里加几条断言。
- C 把「首屏」写成 spec 里的定性要求，靠人工看截图。

**选定 A 的理由**：独立成闸门是因为它的**判定维度**与 smoke 不同——
smoke 判「功能是否可用」（点了有没有反应），本 probe 判「信息是否可达」
（看见它要滚多远）。混在一起会让失败含义模糊（一个闸门一个判据，同 D-135a）。

关键设计是**契约驱动定位**：先用 HTTP 拿 `warm.one_liner` 的真实文本值，
再拿这个字符串去 DOM 里找元素。这样它不依赖任何 class 名——
D-145a 的教训（probe 不得把对方内部命名钉成契约）在这里被直接复用。

**否决 B 的原因**：smoke 已有 37 用例、职责是功能可用性，再塞版面判据会让它
既慢又杂。且 smoke 的 `btn:*` 循环结构不适合做像素级定位。
**否决 C 的原因**：宪法第一条——「唯一可信的状态来源是可执行脚本」。
本次事故本身就是「spec 写了但没探针」的直接后果：004 判据 1 写的是
「结果区第一节就是对提问的回应」，而实测第一节是排盘坐标、提问回应在第 115 屏。
**写在 spec 里但没有断言的判据，等于没有。**

**实测验证本 probe 有效**：首跑退出码 1，报出 6 项未达标，
其中 4 项是真实版面问题、1 项引出 R128a-01（同一批古籍渲染两次）。
若它当时报 PASS，那才是问题。

## D-154a 判据不得隐含规定实现方式；探针的隐藏判定必须覆盖多种折叠实现（R129a）

**背景**：优化轨 R184b 在施工前先模拟目标形态，报回四条问题。亲验后**四条全部
成立**，其中三条源于我自己：

**问题一：判据 1 把「可见性」写成了「文档绝对位置」。**
我写「一句话结论文档绝对 y ≤812px」。实测 `#result` 自身绝对 y 就是 3,005px
（品牌 55 + 今日卡 577 + 功能卡 618 + 最近/收藏 600 + 表单 888）。
即使结果区第一个像素就是那句话，也是阈值的 3.7 倍。

这个判据**隐含要求前端删掉首页的品牌区、今日运势卡、功能卡**——而那些是
`specs/004` 判据 11「今日聚合入口」明确要求存在的东西。**两条判据互相矛盾，
而矛盾来自我用错了度量基准。**

**问题二：探针的隐藏判定只认一种折叠实现。**
我用 `offsetParent === null || height === 0` 判「默认不可见」。实测
`<details>` 关闭态下子元素 **height=300px、offsetParent 非 null、display=block**
（浏览器只是不绘制）。若优化轨用 `<details>`（HTML 原生折叠、无障碍友好、
零 JS），判据 3/4 会永远 FAIL——**探针在强迫对方改用 `hidden`。**

**问题三：段数统计重复计数。** 父容器与子元素各计一次，报出「48 段、占比 188%」。
占比超 100% 本身就是 bug 的证据，我第一版没察觉。

**候选方案**：

- **A（选定）** 三处都改探针与判据，不改要求对方的行为：
  判据 1 改量**相对提交后视口**的偏移，并在 spec 里明写「上浮/滚动定位/独立视图
  皆可，做法由优化轨定」；`isHidden()` 覆盖 details/hidden/display/visibility/
  content-visibility 五种情形；段数只取最内层命中节点但保留多次渲染的计数能力。
- B 在 spec 里规定「必须用 hidden，不许用 details」「必须把首页各块收起」。
- C 放宽阈值（判据 1 从 812px 改成 4,000px）。

**选定 A 的理由**：**闸门管行为，不管做法**（D-145a 已立此原则，本条是它的第二次
应用）。「用户提交后无需滚动就能看到结论」是行为；「用 hidden 还是 details」
「用上浮还是滚动定位」是做法。把做法写进判据有两个害处：一是限制对方的技术选择
（`<details>` 其实是更好的选择——原生、无障碍、零 JS），二是**判据会与其他判据
打架**（如上面判据 1 与 004 判据 11 的矛盾）。

段数那条尤其值得记：**当一个占比算出超过 100% 时，先怀疑自己的算法，不要怀疑
被测对象。** 我差点把「188%」当成缺陷严重程度的证据报出去。

**否决 B 的原因**：越界。而且实测 `<details>` 方案更优（原生折叠 + 无障碍），
规定用 `hidden` 反而是让对方用更差的方案。
**否决 C 的原因**：放宽阈值是「为了让数字变好而放宽闸门」的变体（红线第 2 项）。
4,000px 意味着用户仍需滚近 5 屏，那等于承认判据没用。正确做法是修**度量基准**，
不是修阈值——基准对了，812px 这个阈值本身是合理的（一屏之内）。

**这条同时是自我纠错记录**（宪法第一条）。四条反馈里有三条是我的问题，
而它们全部由优化轨在施工前的模拟中发现——**它没有闷头照着错判据做，而是先测
再报**。这个做法应当鼓励：若它照 spec 硬做，最后会交出一个删掉首页入口、
禁用 `<details>`、且仍然过不了假阳性判据的实现，双方都要重来一轮。

## D-155a 探针要能区分「折叠」与「删除」；而修法在探针侧，不在实现侧（R130a）

**问题（优化轨 D-240b 报出，我亲验成立）**：`probe_first_screen` 用 `innerText`
定位引文。实测关闭态 `<details>` 内元素 **innerText 长度 = 0**、textContent = 32。
于是：

    机制 A（<details> 折叠）→ 定位 0 段 → 判据 4 报 0% → PASS
    机制 C（真把原文删掉）  → 定位 0 段 → 判据 4 报 0% → PASS

**两者在探针眼里完全一样。** 而这两件事一件合规、一件违反宪法第三条
（引用必须可追溯印本/底本）。**探针把唯一要守的那条线放过了。**

**优化轨的处置是「保留 hidden，因为 hidden 下探针能报阳性证据」。我不采纳。**
那是让实现迁就探针——正是 D-154a 刚纠正的错误（闸门管行为、不管做法）。
而且它的代价是放弃 `<details>`：原生折叠、无障碍友好、零 JS，是更好的实现。

**候选方案**：

- **A（选定）** 改探针：定位从 `innerText` 换成 `textContent`（不受
  display/details 影响），两种机制都能定位到；再由 `isHidden()` 判可见性。
  **另立判据 4b「折叠 vs 删除」**：定位段数必须 ≥ API 返回段数。
- B 采纳优化轨方案，在 spec 里要求用 `hidden` 不许用 `<details>`。
- C 只改定位方式，不加 4b。

**选定 A 的理由**：判据 4 与 4b 回答的是**两个不同问题**，必须分开——
判据 4 问「可见的古籍是不是太多」（占比 ≤40%），判据 4b 问「不可见的那些是否
还在」（段数 ≥ API）。合成一条就会出现「0% 可见」这种既可能最优、也可能最糟的
读数。一个闸门一个判据（同 D-135a、D-153a）。

**否决 B 的原因**：越界，且逼对方用更差的实现。这已是同一错误的第三次苗头
（D-145a 命名、D-154a details/绝对 y、本条），说明「探针不得规定实现」这条
需要作为固定检查项：**每次写判据时问一句「它是否只有一种实现方式能通过」，
若是，判据大概写错了。**
**否决 C 的原因**：只改定位，则「定位到 0 段」仍然会被判据 4 当成 0% 放行。
必须有一条断言明确要求段数对齐，否则删除永远可以伪装成折叠。

**阳性对照（宪法第三条偏离 4：没有阳性对照的闸门等于没有闸门）**，三形态实测：

    <details> 折叠：定位 3/3、折叠 3 → PASS（期望 PASS）✅
    hidden 折叠：   定位 3/3、折叠 3 → PASS（期望 PASS）✅
    真删 2 段：     定位 1/3        → FAIL（期望 FAIL）✅

**副产品**：`<details>` 与 `hidden` 现在都能过，优化轨恢复了实现自由。
已明确告知它不必为探针保留 `hidden`。

**待办（记在此以免遗忘）**：优化轨指出「单案例验证正是让 L1 漏过的原因」——
它已改用五案例 fixture。**我的 probe 目前也只跑单案例（1998-07-20 女）**，
同族风险。下轮应扩到多案例（不同生日/有无提问/不同功能），
否则我可能在某个未测案例上放过缺陷。

## D-238b R185b 决策：古籍折叠机制选 `hidden` + `<button>`（不是 `<details>`）

**背景**：005 要把古籍从默认正文降为可展开证据。折叠机制有四个候选，
实测（Chromium 151.0.7922.34，12 段 × 4,000 字注入对照）：

    机制                       折叠后 12 段总高  offsetParent  rect.height  textContent
    <details> 关闭             307px            非 null       6,040px      可取
    hidden 属性                768px            null          0            可取
    display:none               768px            null          0            可取
    content-visibility:hidden  768px            非 null       0            可取

关键实测：**`<details>` 关闭态下子元素仍有 6,040px 的布局盒**（浏览器只是不绘制），
`innerText` 长度为 0 但 `textContent` 完整。

**这个事实一度让我误判**。我先报「`<details>` 不满足探针的隐藏判据」，
后又报「`<details>` 下探针定位到 0 段 → 判据 4 是空过，无法区分折叠与删除」。
第二条被审查轨采纳（D-155a），但**它的修法在探针侧而非实现侧**：R130a 把定位
从 `innerText` 换成 `textContent`，并新增判据 4b（定位段数 ≥ API 段数）。
修完之后**两种机制都能过，且都能被正面核验**——审查轨明确恢复了我的实现自由。

**最终仍选 `hidden` + `<button>`，理由与探针无关**：

1. `.cite-body` 需要 `white-space:pre-wrap` 保留古籍原文换行（判据 8），
   而 `<details>` 的 `<summary>` 带一套默认样式（marker、display 特性），
   要压平它们比自己写 button 更啰嗦。
2. 三级折叠里每级都要显示可变摘要（「12 段 / 7 部书」「《穷通宝鉴》 3 段」
   「出处 · 因「月柱」被选中 · 4,123 字」），button + span 的结构更直白。
3. `<button aria-expanded>` 的键盘可达与屏幕阅读器语义是原生的，
   与 003 的 `--tap` 44px 令牌直接兼容（`.warm-basis summary` 那处已有先例
   说明 summary 要额外写 `min-height` + `display:flex` 才能满足点击区）。

**代价与否决项**：不用 `<details>` 就得自己管 `aria-expanded` 与 caret 状态
（`toggleCite()` 12 行）。可接受。
**没有采用**「为迁就探针而选机制」的思路——那是 D-154a/D-155a 刚纠正的方向。

## D-239b R185b 决策：判据 1 用「提交后滚动定位」，不隐藏首页各块

**初版方案（已废弃）**：判据 1 原写「一句话结论文档绝对 y ≤812px」，
而 `#result` 自身绝对 y 实测 3,005px（品牌 55 + 今日卡 577 + 功能卡 618 +
最近/收藏 600 + 标题 43 + 表单 888）。我据此设计了 `focusResult`——
提交后把首页各块置 `hidden` + 插返回条，实测能把 `#result` 压到 136px。

**审查轨 R129a 订正了判据口径**（D-154a）：改量「相对提交后视口」的偏移，
理由是「把绝对 y 当判据等于隐含要求前端删掉首页各块，那是 spec 规定实现方式」。
**订正之后 `focusResult` 的唯一存在理由消失了**，我随即废弃它。

**现方案**：提交成功后 `window.scrollTo({top: resultTop - 8, behavior:'auto'})`。
实测三种定位方式：

    什么都不做（仅折叠+反转）  L0 视口 y = 587–793px  过，但取决于表单高度
    显式滚到结果区顶部          L0 视口 y = 353px      过，确定
    滚到一句话本身              L0 视口 y = 60px       过度（四柱被推出屏）

选中间那个。「什么都不做」也能过但是**偶然**——表单加一行就可能超 812px。

**附带收益（这条是选它的实质理由，不是安慰）**：首页 `.brand` /
`#dailyCard` / `#funcGrid` 一个都不隐藏，spec 那条「不得把今日入口永久藏死」
（判据 11 的前置）天然满足，不需要任何复原路径去保证——**不存在的机制不会出 bug**。

`behavior` 用 `auto` 不用 `smooth`：reduced-motion 下不产生动画（判据 18），
也避免闸门在滚动未完成时取到中间值。

## D-240b R185b 决策：古籍区用「单层总折叠」，按书组头平铺方案被实测推翻

**被推翻的方案（我自己的 V6/L1）**：按书分组，每部书一个 44px 组头直接平铺
在结果区里。我测了两个用例就下了结论：

    7 部书 → 2,991px（余 257px）　8 部书 → 3,187px（余 61px）
    推断：「书数越多越高，最坏 8 书仍有余量」→ 选定 L1

**第三个用例把它打死了**：6 部书（1985-12-03 女 事业运）实测 **3,288px = 5 屏**，
判据 2 要 ≤4 屏（3,248px），**超 40px FAIL**。

两个根因，都是我没测就假设了：

1. 组头文本在 375px 视口下**换行**——长书名占两行，实测 492/8 = 61px 而非
   设计的 44px。
2. 那一案的**白话段本身更长**（十神格局 494px 等），与书数无关。

**即：高度不随书数单调，我拿两个点推出的趋势是错的。** 这正是 D-150a
（判据不能建立在会漂移的量上）说的同一类错误，我在自己的选型里犯了一次。

**现方案（L2）**：一个 44px 总入口「📜 古籍原文依据 · 12 段 / 7 部书」，
展开后才是按书分组 → 每段 → 原文。**古籍区默认高度与书数、段数完全无关**，
五案实测恒为 44px。

    用例（书数）        result_h   余量
    c7_love    (7 书)   2,619px   629
    c8_love    (8 书)   2,752px   496
    c6_career  (6 书)   2,980px   268   ← L1 在此案 −40px FAIL
    c6_health  (6 书)   2,640px   608
    c8_noq     (8 书)   2,441px   807

**同时把教训写进闸门**：`web/check_plain_first.py` 跑**五个**用例而非一个，
并额外断言「判据 2 余量 ≥200px」——边缘通过（L1 的 +61px）在别的用例上就是 FAIL。
单用例验证正是让 L1 漏过的原因。

## D-241b R185b 决策：判据 8 按多重集比对，不按下标

`check_plain_first` 第一版用下标逐段比对「展开后原文 vs fixture」，五案全报
FAIL（如「长度 1245 vs 887」）。**是闸门写错了，不是实现错了**：

渲染按书分组，同一部书的段落被收拢到一起，**DOM 顺序因此与 API 顺序不同**。
判据 8 要求的是「每段原文逐字节一致」（内容不被改写/截断），
**不是「顺序与 API 相同」**——把分组误报成内容漂移。

改为按多重集（`sorted(got) != sorted(want)`）比对，并在报错时列出
「DOM 独有 / fixture 独有」两侧。截断、改标点、丢段、重复段仍然全部能抓到
——阳性对照实测：注入删 2 段 → 判据 4b/5/6 命中；注入全展开 → 判据 2/3/4 命中；
注入改一字 → 判据 8 命中。三种注入全被抓（宪法第三条偏离 4：
没有阳性对照的闸门等于没有闸门）。

## D-156a 提问对检索零影响立为 MAJOR，但明确禁止顺手改检索（R131a）

**实测事实**（7 提问变体 × 同一生日，审查轨亲验）：

    引文集合只有 1 种，12 段逐字节相同（含「无 question」与「空串」）
    why 取值域 = ['年柱', '日柱', '月柱']
    retrieve_fast(b, per_query=2, per_work=1, top_queries=3) → 无 question 参数
    services.py:159 调用处也没传

`warm.one_liner` 会随提问变化，但那是**文案模板**在响应，**检索层从未看见提问**。

**为什么这是 MAJOR 而不是 MINOR**：`question` 是 API 入参、前端专门有输入框在
收集它、文案还会引用它（「你问「感情运怎么样？」——」），用户由此合理预期它影响
证据选取。而实测它对证据零作用。这是**契约错误**——不是「效果不够好」，
是「声称做了但没做」。

宪法第三条架构图把 Agent 层定义为「**检索即推理**：判型→选书→读→扩展→验证」。
当前实现是「按盘取书」，推理那一环不存在。R128a 实测带出的「遁甲演義」
`王璋曰乙竒臨乾驚門庚辛囚死乃木入金鄉也`、「太乙金鏡式經」正是这个缺失的表现——
它们与「八字感情运」无关，但与盘位有关。

**候选方案**：

- **A（选定）** 立为 MAJOR 写进 `AUDIT_FINDINGS`，**并在条目里写明改法的约束**：
  须单独一轮、附 eval_g1/eval_g7 前后对照，不得为了让相关性变好而放宽这两个闸门。
- B 立为 MAJOR，要求本轮一并修。
- C 降为 MINOR 进 backlog（理由：不影响功能可用）。

**选定 A 的理由**：改检索会**移动 eval_g1 与 eval_g7 的门柱**——
eval_g1 是 248 题检索评测（当前 246/248 PASS）、eval_g7 是拒答评测
（must_refuse 30/30、must_answer 25/25、FABRICATIONS 0）。
证据选取逻辑一变，这两个评测的输入分布就变了，很可能出现「相关性主观上更好、
但 eval 分数下降」的局面。那时最危险的动作是**顺手放宽 eval 阈值**——
正是红线第 2 项。所以立案时就把约束写进条目，比等到那一轮再讨论更安全。

优化轨的处置也正确：它调查出结论、写进台账建议我立案、**没有自己动
`src/guji/`**——它明确说了「changing retrieval would move the eval_g1/g7
goalposts and deserves its own round」。这个判断与我一致。

**否决 B 的原因**：会把「版面修复」与「检索重构」混在一轮，两者的回归面完全
不同（前者只动前端、后者动计算层与评测）。混合后若 eval 掉分，无法判断是哪一侧
造成的。
**否决 C 的原因**：`question` 是用户可见的输入。一个「填了没用」的输入框比
没有这个输入框更糟——它承诺了不存在的能力。这不是体验瑕疵。

**给下一轮的前置要求**（写在此以免遗忘）：修这条时须先确定
「相关性」的可测量判据（如提问关键词与引文的命中关系、不同提问的引文集合差异度），
否则无法判断改动是否真的改善——同 D-143a：先有量尺，再动刀。

## D-242a LLM 接入形态：服务端接入（否决 BYOK 与不接）（R187b，2026-08-21）

**背景**：用户拍板接入 LLM 并直接提供 key（agnes API）。D-146a 原案为
BYOK（key 只存浏览器、服务端零 key），其前提「成本顾虑」已不存在。

**候选方案**：
- A 服务端接入（选定）：key 放服务端 gitignored 配置，页面无感调用。
  理由：用户给 key 即拍板信号；BYOK 把「配 key」门槛转嫁给小红书目标用户，
  与产品方向矛盾。D-146a 其余六条约束（不入库/显著标注/不可复现告知/
  确定性不被替代/零引用外观/可一键关）全数保留，安全面无净损失。
- B 严格 BYOK：体验差、目标用户不会配 key，否决。
- C 不接 LLM：违背用户明确指示，否决。

## D-243a HTTP 客户端：httpx 直连（否决安装 openai SDK）（R187b）

chat/completions 就一个 POST；venv 已有 httpx。装 SDK 是零收益的新依赖，
撞宪法红线第 3 项的边缘。选 httpx 直连，零新增 pip 依赖。

## D-244a 失败策略：静默降级 + 内部重试（R187b）

实测 agnes 端点约半数请求返回空正文或 SSL 断连（随机，与提示词无关）。
策略：polish() 内部重试 3 次；仍失败返回 None，前端整块不渲染。
理由：LLM 是附加层不是承重墙——它的失败不该被用户看见；
报错占位会让断网体验比改造前更差。重试不破坏确定性语义：
该函数本就因 LLM 而不可复现。

## D-245a 环境总开关 BOOKS_LLM_DISABLE（R187b）

ui_smoke 的单用例预算 25s 装不下 LLM 往返（实测 13–30s）。
probes/ 是审查轨领土不能改，故开关落在被测代码侧：
BOOKS_LLM_DISABLE=1 时 load_config() 直接返回 None。
语义：=1/on/true/yes 禁用；未设置按配置文件；闸门环境与用户一键关共用。


## D-246b R190b 决策：merge 方向选「main ← audit 先合」（否决反向与 cherry-pick）

**背景**：双轨在 `2cbb1f8` 后分叉，两侧 `AUDIT_FINDINGS.md` 各持一半真相，
`count_open_findings.py` 在两边都 exit 1，各自卡着对方已解决的那一条。

**候选方案**：
- **A（选定）main ← audit，再 audit ← main**。main 是产品主线且领先 3 个提交，
  先把审查轨的验收结论拉进来，在 main 上完成状态流转与文档订正，再让 audit
  快进。冲突只在 append-only 文档，按宪法第五条两段都保留。
- B audit ← main 先合。否决：审查轨会先拿到**尚未订正**的矛盾文档，等于把矛盾
  复制一份，之后两边都要改同样的地方。
- C 手工 cherry-pick audit 的文档段落，不 merge。否决：丢失提交祖先，此后每次
  merge 都会重复同一冲突，且违背「不改写历史」的红线精神。

**执行前置**：`git branch backup/main-pre-merge-R190b`、
`backup/audit-pre-merge-R190b` 两个备份分支已建（回滚路径 = 一条 reset）。

## D-247b R190b 决策：R131a-01 标 `FIXED-R189b` 而非 `VERIFIED`（否决自签与保持 OPEN）

**候选方案**：
- A 直接标 VERIFIED。三判据我自己跑过全 PASS，但宪法第一条 NON-NEGOTIABLE
  写明「修复方不得自己宣布完工」，本轮同时在改代码与文档属修复方。**否决**。
- B 保持 OPEN。闸门 1 永远 FAIL，两个新窗口开机即自锁（优化轨指令要求
  「push 前本地全量闸门绿」）。**否决**。
- **C（选定）** 标 `FIXED-R189b` + 把独立复现脚本、实测输出、以及
  「转 VERIFIED 需要新审查轨自跑哪四条命令」全部钉进条目。
  FIXED 不计入 OPEN，闸门解锁；同时新审查轨能逐条推翻我。

**理由**：这样「解锁」与「可被推翻」同时成立。若只做 A，纪律破口；只做 B，
项目停摆；C 把信任问题转成了可执行命令问题——这正是宪法第一条的本意。

## D-248b R190b 决策：新建闸门一律自带 `--self-check` 阳性对照

**背景**：本轮补建 4 个闸门脚本（check_poster、probe_llm_polish、
probe_r131a_relevance、probe_r128a_no_dup_citations）。

**候选方案**：
- A 只写主判据。否决：U-08 教训——`probe_bcv.py` 曾**永远返回 0**，包括它打印
  「56/66 卷」的那一次。没有阳性对照的闸门等于没有闸门（宪法第三条偏离 4）。
- **B（选定）** 每个脚本内建 `--self-check`：注入一个**已知会坏**的情形，
  断言判据必须 FAIL；若仍全绿则脚本自报「本闸门是假的」并 exit 1。

**实测**（四个全部有效）：check_poster 抹水印+改尺寸 → 判据 12 FAIL；
probe_llm_polish 打桩「超时也返回文本」+ 净化失效 → 判据 2 四条与判据 7 FAIL；
probe_r131a 打桩 `topic_queries→[]` → 5 种集合塌成 1 种；
probe_r128a 克隆 `.cite-body` → 12 变 24。

## D-249b R190b 决策：`probe_llm_polish` 的真实调用用例走 `--online` 开关，不进闸门清单

**候选方案**：
- A 判据 1（四端点 ai_polish 非空）默认跑真实 LLM。否决：闸门必须可离线复现，
  否则断网/端点抖动会让闸门变成掷硬币（agnes 端点实测约半数请求空正文或断连，
  见 D-244a）。且单次往返 13–30s，会把闸门时长拖垮。
- **B（选定）** 默认只跑 offline 判据（2/2b/3/6/7/8，全部零联网）；判据 1 由
  `--online` 显式触发，且**其结果不参与退出码**，只打印。

**实测**：`--online` 四端点 ai_polish 全非空，且正是这次运行抓到了 B-016
（AI 称「林先生」而入参是「女」）——说明 online 用例有价值，但价值在于
**人工排查**，不在于当闸门。

## D-250b R190b 决策：越界改审查轨文件，但以「显式标注 + 零改写」为代价换取解锁

**背景**：宪法第五条把 `docs/PHASE.md`、`docs/AUDIT_FINDINGS.md`、
`docs/OPTIMIZE_BACKLOG.md`、`probes/**`、`specs/*/spec.md` 划为审查轨独占写。
但本轮必须动它们——它们的自相矛盾正是阻塞两个新窗口开工的东西。

**候选方案**：
- A 严守领土，只写一份「建议清单」等审查轨来改。否决：审查轨此刻无人在跑
  （DSH 会话已断），用户正要开新窗口，等于把死锁留给新窗口第一分钟。
- B 直接改，不声明。否决：这正是宪法第一条最痛恨的「偷偷改掉」。
- **C（选定）** 改，但四条自我约束：(1) 每处改动以「R190b 订正/补记」显式标注；
  (2) 绝不改写任何既有条目或结论，只追加；(3) 不动 `CURRENT_PHASE` 那一行
  （它仍是 R120a 翻的 OPTIMIZE）；(4) 在台账 §128 与本条各声明一次，
  并把「复核 R190b 有无越界」列为新审查轨的第一批任务之一。

**理由**：领土纪律的**目的**是「需求被写成文字而不是被偷偷实现掉」
（用户原话）。显式标注 + 零改写 + 主动请审，比沉默守界更贴合这个目的。

## D-251b R191b 决策：B-014 修法选「后台线程 + 轮询取回」（否决同步短超时与 SSE 流式）

**背景**：B-014 实测（R191b 复现）：LLM 开启时 `/api/bazi` 端到端 **35.3s**
（call1=0.8s，方差极大），根因是请求线程内串行 `polish()`（内部重试 3 次 ×
timeout_s=30，最坏 90s）。用户原话直指「治愈、低门槛」——30 秒白屏直接对撞。

**候选方案**：
- A 同步保留、首 attempt 缩到 3–5s。否决：agnes-2.5-flash 是推理模型，
  R187b 实测延迟 1.5s–45s 波动，3–5s 内能出文的概率低——AI 段落会从
  「偶尔慢」变成「几乎永远拿不到」，等于变相砍掉 LLM 层；且同步窗口内
  仍可能撞 5s 白屏。
- B SSE / WebSocket 流式。本轮否决：spec §6 明确不做流式是既有决策，
  推翻它的证据只支持「异步化」，不支持引入新协议形态；SSE 需要
  TestClient 流式读取配合、probe_ui_smoke 的点击断言模型也要跟着改，
  改动面大且对「2–3 句一段话」的产物没有增量价值。
- **C（选定）确定性主体同步返回；AI 段落交后台 daemon 线程生成；
  响应附 `ai_task_id`；新增 `GET /api/ai/{id}` 轮询端点；前端渲染完
  确定性主体后轮询（500ms 间隔、上限 ~40s），拿到就插入 `.ai-polish`
  容器，`{"status":"failed"}|404|超时` 就整块不渲染。**

**降级语义不变（红线）**：拿不到 AI 文本 = 整块不渲染，与 D-244a 一致。
DISABLE=1 时零行为变化（无 ai_task_id 键），闸门环境逐字节不变——
这是判据 11，由 check_async_ai 断言。

**三库零入库不变**：任务表在进程内存 dict（重启即失，AI 文本永不落盘）。

**代价与接受理由**：轮询有最多 ~80 次额外 GET（40s 上限）；相比 SSE
实现复杂度与契约探针改造成本，可忽略。id 用 `secrets.token_urlsafe(16)`
+ 进程内 dict，单用户桌面应用形态下无跨进程需求。

## D-252b R191b 决策：B-015 修法选「双管齐下」——金行池补女性向字 + 性别分提到缺行命中之前

**背景**：B-015 两因叠加：(a) `_full_name_combos` 排序键
`(-缺行命中, -性别分, 表序)` 让性别分只在同命中数内起作用；(b) 金行池
20 字 FEMININE_CHARS 命中 0——排序怎么调都出不了女名。

**候选方案**：
- A 只调排序（性别分提前）。否决：金字池零女性向字，女生缺金仍然全鑫——
  治标不治本，判据（前 8 含 FEMININE ≥5）过不去。
- B 只补字不动排序。否决：双字组合「两字都命中缺行」恒排最前，补进去的
  女性向金字会被 鑫+X 双缺行组合压在后面，前 8 依然出不来。
- **C（选定）A+B 同做**：金行池补两个钅部首的女性常用字——铃
  （「金-铃音清越」）、钗（「金-金钗之贵」，金陵十二钗的钗）；并把池内已有的
  钰、锦补进 FEMININE_CHARS（两字在真实女名中高频，此前只是漏归类）。
  排序键改为 `(-性别分, -缺行命中, 表序)`，即性别分优先于缺行命中数。

**取舍声明**：性别分优先意味着「林瑶瑶（土土，不补金）」这类**不补缺行但
性别契合**的名字会排在「林鑫铭（补金但男性向）」前面。这是有意的产品决策：
目标用户是小红书 18–30 女性，「名字好听像自己」优先于「五行补齐的教条」；
缺行信息仍在 candidates 与 summary 中完整展示，专业信息不丢失。
单字名形态（hit=0 或 1）同样受惠：女性向金字直接顶到最前。

**确定性**：排序键仍是纯函数（无 random/时钟），两次调用逐字节相等不变。

## D-253b R191b 决策：B-016 修法选「services 层传 gender 进 facts_qiming」，并越界改 probe_llm_polish --online 加称谓断言

**背景**：B-016 根因 `facts_qiming(q)` 不喂性别，模型自己猜出「林先生」
（入参 gender=女）。同型风险：facts_taohua/facts_hehun 也不喂性别
（合婚两人性别都有），本轮一并补上——治类不治点。

**候选方案**：
- A 在提示词里加一句「请勿使用先生/女士称谓」。否决：治症状不治根因——
  模型不知道该用哪个称谓，只会换一种方式猜。
- B name_candidates 返回值回填 gender。可行但要动纯函数返回形状，
  selftest/probe 可能钉了返回 dict 的键集；收益不如 C 直接。
- **C（选定）services.qiming 把 req.gender 作为第二参传入 facts_qiming(out, gender)；
  facts_qiming 增加「性别：女/男」事实行。taohua/hehun 同理补性别行。**

**领土偏离声明（宪法第五条）**：判据要求把称谓断言写进
`probes/probe_llm_polish.py --online`——该文件属审查轨领土。处置照
D-250b 先例：改动最小化（check_online 函数内加两条断言）、显式标注
「R191b 补记」、不改任何既有断言、台账 §129 声明一次、列审查轨复核。

## D-254b R194b 决策：首页 IA 选「带标签的三簇语义分组」，簇内序按用户意图谱系（否决纯重排与五簇细分）

**背景**：§131 移交的「首页 IA」无任何方向描述；8 张功能卡平铺，
问事/学习、单人/双人意图差异在版面零表达。spec 007 §2。

**候选**：
- A. 纯重排不加标签——零新概念但「分组」不可感知，判据无法自动化
  （顺序本身无语义锚点），放弃。
- B. 五簇细分（今日/问事/起名择日/读书/双人）——每簇 1–2 卡，
  375px 下标签数接近卡数，分组反而增加扫视成本，且「黄历择日」
  归「今日」还是独立簇两可。放弃：粒度过细违背分组初衷。
- C.（选定）三簇：今日=每日运势卡本体（不新增壳）、测一测 7 卡、读书 1 卡。
  簇内序按语义谱系：八字族（排盘→起名→合婚→桃花）相邻——同源于四柱；
  抽问族（塔罗→六爻）相邻；黄历殿后；合婚与桃花保持相邻（两人关系意图）。

**约束**：入口不减不藏不折叠（8 张全保留，data-view 零变化——
probe_ui_smoke 全部用例经 .func-card 导航）；键盘序=视觉序；
...
## D-258b R206b 决策：009 目标用户聚焦重塑——功能大审判「收进高级抽屉」而非删除 + AI 陪伴层走既有异步管道

**背景**：用户 2026-08-23 产品级批评——读书视图的研究型功能（书 ID/编址率/
比对/线程等）对 15–25 岁目标用户是噪音；「一切以目标群众的实际需求改动」；
并授权加入 AI 情绪价值层（agnes 端点，与 web/llm_config.json 现有配置一致）。

**裁决（specs/009-audience-focus/spec.md §2 三栏表）**：
- ① 研究型功能**全部收进首页「高级抽屉」**，零删除——路由/API/闸门/探针
  原样保留，只动入口。否决「直接删代码」：违反双轨隔离精神（内容芯是
  审查轨质量体系的承重对象，闸门与探针钉着它们），删除=34 项闸门全红
  且不可回滚；收抽屉=小满看不到、老玩家可达、闸门照跑。
- ② AI 陪伴层**复用 llm_polish 异步管道**（spawn+轮询骨架、agnes 配置、
  facts_* grounding），additive 新端点。否决「新起一条同步聊天管道」：
  D-251b 已验证异步模式，B-014 的 29–32s 阻塞教训不重蹈。
- ③ 对话**不写库**：零隐私留存，与 history.db 隔离；会话设轮数上限
  （防依赖设计，目标用户保护条款）。
- ④ 排盘表单减负走**前端默认值**（后端 schema 零改动，契约不动）。

**可测量性**：specs/009 §3 各 US 判据——首页研究型元素 DOM 计数=0、
DISABLE=1 降级路径、禁语清单扩容零命中、E2E 对话 grounding 关键词断言。

**大改三步前置（宪法用户职权条款）**：量尺=本 spec §3 判据；
