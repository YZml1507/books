...
> **R191b 处置（2026-08-22）：已修，审查轨复验通过 → VERIFIED-R133a（见 AUDIT_FINDINGS R133a-01）。**
> 复现基线：R191b 实测 call0=**35.3s**（比登记的 29–32s 更糟）/call1=0.8s。
> 修法：后台线程 + `GET /api/ai/{id}` 轮询（D-251b），确定性主体立即返回。
> 改后实测：真实 agnes 下 POST **0.31s**、轮询 #17 拿到文本；打桩闸门
> `web/check_async_ai.py` 判据 9/10/11 全 PASS + `--self-check` 阳性对照有效。
> specs/006「不做流式」已按合法程序修订（spec §2/§6 + tasks.md 同步）。
...
> **R191b 处置（2026-08-22）：已修，审查轨复验通过 → VERIFIED-R133a。**
> 修法「双管齐下」（D-252b）：金字池补 铃/钗 + FEMININE_CHARS 补录 钰/锦；
> 排序键改为 `(-性别分, -缺行命中, 表序)`——性别契合优先于补缺教条
> （取舍已声明：目标用户 18–30 女性，「像自己的名字」优先，缺行信息仍在
> candidates/summary 完整展示）。实测：女（林·缺金）前 8 全名
> 林锦钰/林鑫锦/林鑫钰/林铭锦/林铭钰/林铮锦/林铮钰/林锦钟 = **8/8** 女性向命中；
> 男（王·缺木）前 8 王柏栋…王柏荣 = **8/8** 男性向命中；两次调用逐字节相等。
> 判据进模块自测：`PYTHONPATH=src <py> -m guji.qiming` exit 0。
...
> **R191b 处置（2026-08-22）：已修，审查轨复验通过 → VERIFIED-R133a。**
> 修法（D-253b）：`facts_qiming(out, gender)` 显式喂「性别：女/男」事实行；
> facts_taohua/facts_hehun 同型补齐（taohua 喂本人性别、hehun 喂双方性别）——
> 治类不治点；系统提示词加「称谓必须与性别事实一致」。称谓断言已进
> `probe_llm_polish --online`：gender=女 →「先生」零命中、gender=男 →
> 「女士/小姐」零命中（该文件为审查轨领土，按 D-250b 先例最小修改 + 显式
> 「R191b 补记」标注 + 判据本体未动，台账 §129 已声明）。真实 agnes 复测：
...
> **R192b 处置（2026-08-22）：已修，审查轨复验通过 → VERIFIED-R133a。**
