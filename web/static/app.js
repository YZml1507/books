  ...
  switch (view) {
    case 'daily':
      return { title: '今日运势', subtitle: (j && j.date) || '',
        ...
        lines: [{ k: '运势等级', v: ((j && j.level) === '凶' ? '缓' : (j && j.level)) || '—' },
                { k: '天乙贵人', v: (j && j.noble) || '—' },
                { k: '宜', v: (j && j.do) || '—' },
                { k: '忌', v: (j && j.dont) || '—' }],
        cards: [], view: view };
...
var _HL_COMPLEX_DATE = /农历|農曆|阴历|陰曆|旧历|舊曆|闰|閏|正月|冬月|腊月|臘月|除夕|春节|春節|大年初一|元宵|端午|七夕|中秋|重阳|重陽|腊八|臘八|清明|立春|雨水|惊蛰|驚蟄|春分|谷雨|穀雨|立夏|芒种|芒種|夏至|处暑|處暑|白露|秋分|寒露|霜降|立冬|冬至|大暑|小暑|元旦|新年|情人|植树|植樹|愚人|劳动|勞動|五一|青年|儿童|兒童|六一|建党|建黨|建军|建軍|教师|教師|国庆|國慶|万圣|萬聖|平安|圣诞|聖誕|跨年|母亲节|母親節|父亲节|父親節|感恩/;
...
function initDivination() {
  ...
  var _ENTER_SUBMIT = {
    ...
    'view-hehun': 'hhSubmit'
    /* view-bazi 是真 <form>，Enter 原生已提交；view-huangli 的输入框
     * 是动态注入的（hlAskInput 已有自己的 Enter 绑定）。 */
  };
  document.addEventListener('keydown', function (e) {
