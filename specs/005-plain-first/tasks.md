...
- [ ] T0.1 `docs/DECISIONS.md` 落 **D-240b**（折叠机制为何仍选 `hidden`）：
  ...
  **同一判据下 `hidden` 的通过可核验、`<details>` 的通过是空的**　状态：TODO
- [ ] T0.2 `docs/DECISIONS.md` 落 **D-241b**（L1 按书平铺被推翻）：
  ...
  **6 书 3,288px(−40) = 5 屏 FAIL**；L2/L3 三案分别 2,607/2,739/2,968px。
  ...
  「判据不能建立在会漂移的量上」　状态：TODO
- [ ] T0.3 `docs/DECISIONS.md` 落 **D-242b**（判据 1 用滚动而非隐藏首页）：
  R129a 口径订正后，「什么都不做」实测 L0 视口 y = 587–793px 也能过，
  但那取决于表单高度、属偶然；显式滚到结果区顶部实测恒 **353px**。
  选滚动的附加收益：首页 `.brand`/`#dailyCard`/`#funcGrid` 一个不藏，
  spec 那条「不得把今日入口永久藏死」天然满足　状态：TODO
- [ ] T0.4 台账 §121 登记本轮实测：新探针输出（4 项未达标、可核验性
  两项 ✅、古籍 92%）+ 目标形态五案表（判据 1/2/3/4 全绿，最小余量 280px）
  状态：TODO
- [ ] T0.5 `web/baselines/plain_first_fixture.json`：冻结 5 个用例
  ...
  **必须是多用例**——单用例正是 L1 翻车的原因　状态：TODO

---

## M1 判据 5/6/7/8：去重 + 三级折叠件（清偿 R128a-01）

- [ ] T1.1 `web/static/app.js` 新增 `renderCiteTree(items)`：三级结构
  ...
  三级 `aria-expanded`/`aria-controls` 成对，点击切 `el.hidden`　状态：TODO
- [ ] T1.2 `styles.css` 加 `.cite-wrap/.cite-top/.cite-top-body/.cite-group/
  ...
  **不加** `[hidden]{display:none!important}`——实测不需要（被 hidden 的都是
  普通 div，`getComputedStyle().display === 'none'` 直接成立）　状态：TODO
- [ ] T1.3 去重（判据 5，清偿 R128a-01）：`renderWarm` 不再渲染
  ...
  `citation` 12/12 逐条相等、页面 `.ev-item` 24 个而 API 12 段　状态：TODO
- [ ] T1.4 `buildLiuyaoResult`：`j.ben_jing` / `j.bian_jing` 改走
  `renderCiteTree`（现为 `renderHits` 全展开）　状态：TODO
- [ ] T1.5 专业模式零改动核验：`renderInterpretation` 及其 citations 渲染
  一字不改；`voiceMode()==='pro'` 时 `renderCalc` 位置不变　状态：TODO
- [ ] T1.6 新建 `web/check_plain_first.py`，先实现判据 5/6/7/8：
  ...
  **5 个用例全跑** + **阳性对照**（篡改一字须退出码 1，宪法第三条偏离 4）
  状态：TODO
- [ ] T1.7 里程碑闸门：`<py> probes\probe_first_screen.py`
  （判据 5 转绿，6/7 保持 ✅）+ `<py> web\baseline_voice.py` 退出码 0
  + 复核「折叠后古籍区 = 44px」（实测目标值）　状态：TODO
...
- [ ] T2.1 顺序反转（**仅 warm 分支**）：`buildBaziResult` 改为
  ...
  状态：TODO
- [ ] T2.2 `app.js` 新增 `revealResult(containerId)`：提交**成功后**
  `window.scrollTo({top: resultTop - 8, behavior:'auto'})`。
  用 `auto` 不用 `smooth`（reduced-motion 判据 18 + 避免探针取中间值）。
  **不隐藏首页任何块**（初版 `focusResult` 已取消）。
  目标实测值：L0 相对视口 y = 353px、能量卡 637–880px　状态：TODO
- [ ] T2.3 六爻/塔罗/桃花/合婚/起名/黄历也调 `revealResult`（判据 1 场景 5）。
  后四者实测无 `warm` 键（004 M2 未开工），本轮只保证提交后可见结果 +
  无文本墙 + 不横向溢出，**不新建 warm 文案**　状态：TODO
- [ ] T2.4 `web/check_plain_first.py` 扩到判据 1/2/3/4，**5 个用例全跑**：
  L0 相对视口 y ∈ [0,812]；`#result` ≤3,248px；可见最长块 ≤400 字；
  可见古籍占比 ≤40%。额外断言**余量** `3248 - result_h ≥ 200`
  （防 L1 那种 −40/+61px 的边缘通过）　状态：TODO
- [ ] T2.5 里程碑闸门：`<py> probes\probe_first_screen.py` **退出码 0**
...
- [ ] T3.1 `<py> web\baseline_voice.py`（判据 9，sha256 `b0461df2…`）　状态：TODO
- [ ] T3.2 `<py> web\selftest.py` ≥149 断言（判据 14）　状态：TODO
- [ ] T3.3 `<py> web\check_warm_voice.py` 判据 1–8 全绿（005 判据 17）　状态：TODO
...
- [ ] T3.7 判据 12 自证：切「专业版」= 切回旧版面（`plan.md` §5.2），
  实测切换后 pro 输出逐字节等于基线、`localStorage.voiceMode` 保持　状态：TODO
...
- [ ] T4.3 `why` 上屏（唯一的零风险相关性改动）：第三级摘要行显示
...
- [ ] T5.2 确认 D-240b / D-241b / D-242b 已落 `docs/DECISIONS.md`　状态：TODO
- [ ] T5.3 R128a-01 的状态由审查轨转——我方只在台账写「已清偿 + 复验命令」，
  **不改对方的 `AUDIT_FINDINGS.md` 条目**　状态：TODO
- [ ] T5.4 移交审查轨：本文件状态列全 DONE(命令)　状态：TODO
